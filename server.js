...
function audireTtsPausasLimpa(v){const b=v&&typeof v==='object'?v:{},num=(x,d)=>Number.isFinite(Number(x))?Math.max(0,Math.min(5,Number(x))):d,linha=num(b.linha_em_branco_segundos??b.apos_titulo_segundos,3),frase=num(b.apos_frase_segundos??b.a_cada_duas_frases_segundos,1);return {linha_em_branco_segundos:linha,apos_titulo_segundos:linha,apos_frase_segundos:frase};}
...
function audireTtsPausaEntreSegmentos(tipo,pausas){const p=audireTtsPausasLimpa(pausas);if(tipo==='linha_branca'||tipo==='campo')return p.linha_em_branco_segundos;return p.apos_frase_segundos;}
...
const AUDIRE_TTS_REVISOES=new Map(),AUDIRE_TTS_REVISAO_TTL_MS=2*60*60*1000;
