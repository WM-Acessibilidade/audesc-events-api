...
app.get('/audire/public/repositorios',async(req,res)=>{try{const sb=getSupabase(),cfg=await audireObterConfig();const {data,error}=await sb.from('audire_repositorios').select(audirePublicarRepositorioSelect()).eq('status_publicacao','publicado').order('publicado_em',{ascending:false,nullsFirst:false}).order('criado_em',{ascending:false});if(error)throw error;res.json({ok:true,repositorios:(data||[]).filter(r=>audirePaisHabilitado(r,cfg)),config:cfg})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositórios do Audire.'})}});
...
app.post('/audire/admin/roteiros/:id/imagem',express.raw({type:'application/octet-stream',limit:'4mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeImagemValido(mime))return res.status(400).json({error:'Use imagem JPEG, PNG ou WebP.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_IMAGE_MAX_BYTES)return res.status(400).json({error:'Imagem inválida ou maior que 3 MB.'});const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id}/imagens/${r.id}-${Date.now()}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);const {data,error}=await sb.from('audire_roteiros').update({imagem_url:url,imagem_path:path,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar imagem do roteiro.'})}});
app.post('/audire/admin/roteiros/:id/audio',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeAudioValido(mime))return res.status(400).json({error:'Formato de áudio não suportado. Use MP3, WAV, OGG, WebM, M4A ou AAC.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio inválido ou maior que 20 MB.'});const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id}/audios/${r.id}-${Date.now()}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const {data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar áudio do roteiro.'})}});
