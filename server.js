...
function audireWavParaMp3(wav,bitrate=64){const info=audireLerWavPcm16(wav),kbps=[48,64,96].includes(Number(bitrate))?Number(bitrate):64,lame=require('@breezystack/lamejs'),enc=new lame.Mp3Encoder(info.channels,info.sampleRate,kbps),blocos=[],amostras=Math.floor(info.data.length/2/info.channels),left=new Int16Array(amostras),right=info.channels===2?new Int16Array(amostras):null;for(let i=0;i<amostras;i++){left[i]=info.data.readInt16LE((i*info.channels)*2);if(right)right[i]=info.data.readInt16LE((i*info.channels+1)*2)}const passo=1152;for(let i=0;i<amostras;i+=passo){const l=left.subarray(i,Math.min(i+passo,amostras)),r=right?right.subarray(i,Math.min(i+passo,amostras)):null,mp3=r?enc.encodeBuffer(l,r):enc.encodeBuffer(l);if(mp3.length)blocos.push(Buffer.from(mp3))}const fim=enc.flush();if(fim.length)blocos.push(Buffer.from(fim));const out=Buffer.concat(blocos);if(!out.length)throw new Error('Falha ao converter o WAV para MP3.');return {mp3:out,bitrate_kbps:kbps,sample_rate:info.sampleRate,channels:info.channels};}
function audireTtsComporTexto(r,cfg,partes){const p=audireTtsPartesLimpa(partes),blocos=[];if(p.titulo&&String(r.titulo||'').trim())blocos.push(String(r.titulo).trim());if(p.roteiro&&String(r.roteiro_texto||'').trim())blocos.push(String(r.roteiro_texto).trim());if(p.creditos&&String(r.creditos||'').trim())blocos.push(String(r.creditos).trim());if(p.lema&&String(cfg?.lema||'').trim())blocos.push(String(cfg.lema).trim());return {partes:p,texto:blocos.join('\n\n')};}
function audireTtsPrompt(texto,instrucoes){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes;return `Gere somente a fala correspondente ao conteúdo fornecido. Fale em português brasileiro. Não leia estas instruções, não anuncie seções e não acrescente, omita, traduza, resuma ou reescreva o conteúdo. Preserve nomes, números e pontuação da forma mais fiel possível.\n\nDireção de voz: ${instr}\n\nCONTEÚDO A SER FALADO:\n<<<AUDIRE_TEXTO>>>\n${texto}\n<<<FIM_AUDIRE_TEXTO>>>`;}
