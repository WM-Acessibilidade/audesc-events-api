...
function audireSanitizarLinks(valor){const a=Array.isArray(valor)?valor:[];return a.slice(0,30).map(x=>({nome:limit(x?.nome,160),url:limit(x?.url,1000)})).filter(x=>x.nome&&/^https?:\/\//i.test(x.url));}
