...
function audireWavParaMp3(wav,bitrate=64){const info=audireLerWavPcm16(wav),kbps=[48,64,96].includes(Number(bitrate))?Number(bitrate):64,lame=require('@breezystack/lamejs'),Mp3Encoder=lame?.Mp3Encoder||lame?.default?.Mp3Encoder||lame?.default;if(typeof Mp3Encoder!=='function')throw new Error('O encoder MP3 foi carregado, mas Mp3Encoder não está disponível. Verifique @breezystack/lamejs no Render.');const enc=new Mp3Encoder(info.channels,info.sampleRate,kbps),blocos=[],amostras=Math.floor(info.data.length/2/info.channels),left=new Int16Array(amostras),right=info.channels===2?new Int16Array(amostras):null;for(let i=0;i<amostras;i++){left[i]=info.data.readInt16LE((i*info.channels)*2);if(right)right[i]=info.data.readInt16LE((i*info.channels+1)*2)}const passo=1152;for(let i=0;i<amostras;i+=passo){const l=left.subarray(i,Math.min(i+passo,amostras)),r=right?right.subarray(i,Math.min(i+passo,amostras)):null,mp3=r?enc.encodeBuffer(l,r):enc.encodeBuffer(l);if(mp3.length)blocos.push(Buffer.from(mp3))}const fim=enc.flush();if(fim.length)blocos.push(Buffer.from(fim));const out=Buffer.concat(blocos);if(!out.length)throw new Error('Falha ao converter o WAV para MP3.');return {mp3:out,bitrate_kbps:kbps,sample_rate:info.sampleRate,channels:info.channels};}
