...
function audireTtsPausasLimpa(v){const b=v&&typeof v==='object'?v:{},num=(x,d)=>Number.isFinite(Number(x))?Math.max(0,Math.min(5,Number(x))):d,linha=num(b.linha_em_branco_segundos??b.apos_titulo_segundos,3);return {linha_em_branco_segundos:linha,apos_titulo_segundos:linha,entre_paragrafos_segundos:num(b.entre_paragrafos_segundos,1.5),entre_campos_segundos:num(b.entre_campos_segundos,2),a_cada_duas_frases_segundos:num(b.a_cada_duas_frases_segundos,1.0)};}
...
function audireTtsPrompt(texto,instrucoes,pausas,contexto={}){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,antes=limit(contexto?.antes,700),depois=limit(contexto?.depois,700);let ctx='';if(antes||depois){ctx='\n\nContexto do roteiro para preservar sentido e pronúncia. Este contexto NÃO deve ser sintetizado:'+(antes?`\n[antes] ${antes}`:'')+(depois?`\n[depois] ${depois}`:'');}return `Gere áudio de fala para uma audiodescrição em português brasileiro. Sintetize exclusivamente o conteúdo identificado como TRANSCRIÇÃO A FALAR. Não leia instruções nem o contexto. Mantenha as palavras da transcrição na ordem apresentada, sem acrescentar conteúdo. Preserve rigorosamente a mesma identidade de locutor em todos os trechos desta sessão: mesma voz selecionada, mesmo timbre, mesma faixa vocal e mesma impressão de idade. Varie apenas a prosódia necessária ao texto; não troque de locutor.\n\nEstilo de leitura: ${instr}${ctx}\n\nTRANSCRIÇÃO A FALAR:\n${texto}`;}
...
function audireTtsQuebrarUnidadeLonga(texto,maxChars=650){const palavras=String(texto||'').trim().split(/\s+/).filter(Boolean),saida=[];let atual='';for(const palavra of palavras){const teste=atual?atual+' '+palavra:palavra;if(teste.length>maxChars&&atual){saida.push(atual);atual=palavra}else atual=teste}if(atual)saida.push(atual);return saida;}
function audireTtsSegmentarTexto(texto,maxChars=650){const fonte=String(texto||'').replace(/\r\n?/g,'\n').trim();if(!fonte)return[];const blocos=fonte.split(AUDIRE_TTS_MARCADOR_CAMPO),unidades=[];for(let bi=0;bi<blocos.length;bi++){const bloco=String(blocos[bi]||'').trim(),partes=bloco.split(/(\n+)/);for(let i=0;i<partes.length;i+=2){const trecho=String(partes[i]||'').trim(),sep=String(partes[i+1]||'');if(!trecho)continue;const frases=trecho.split(/(?<=[.!?…])\s+(?=[A-ZÁÀÂÃÉÊÍÓÔÕÚÜÇ0-9“"'])/u).map(x=>x.trim()).filter(Boolean),itens=frases.length?frases:[trecho];for(let j=0;j<itens.length;j++){const pedacos=audireTtsQuebrarUnidadeLonga(itens[j],maxChars);for(let k=0;k<pedacos.length;k++){const fimFrase=k===pedacos.length-1,fimTrecho=fimFrase&&j===itens.length-1;let quebra='frase';if(fimTrecho)quebra=sep.length>=2?'linha_branca':sep.length===1?'paragrafo':'frase';unidades.push({texto:pedacos[k],quebraApos:quebra,fimFrase});}}}if(bi<blocos.length-1&&unidades.length)unidades[unidades.length-1].quebraApos='campo';}const segmentos=[];let atual='',frasesNoAtual=0,quebra='frase';const minimo=Math.max(140,Math.round(maxChars*.60));const fechar=(tipo)=>{if(atual.trim())segmentos.push({texto:atual.trim(),quebraApos:tipo||quebra});atual='';frasesNoAtual=0;quebra='frase'};for(const u of unidades){const teste=atual?(atual+' '+u.texto):u.texto;if(atual&&teste.length>maxChars)fechar(frasesNoAtual>=2?'respiracao':quebra);atual=atual?(atual+' '+u.texto):u.texto;if(u.fimFrase)frasesNoAtual++;quebra=u.quebraApos;if(u.quebraApos==='campo'||u.quebraApos==='linha_branca'){fechar(u.quebraApos);continue}if(u.quebraApos==='paragrafo'&&atual.length>=minimo){fechar('paragrafo');continue}if(frasesNoAtual>=2&&atual.length>=minimo)fechar('respiracao')}fechar(quebra);if(segmentos.length>1){for(let i=segmentos.length-1;i>0;i--){if(['campo','linha_branca'].includes(segmentos[i-1].quebraApos))continue;if(segmentos[i].texto.length<110&&(segmentos[i-1].texto.length+1+segmentos[i].texto.length)<=Math.round(maxChars*1.35)){segmentos[i-1].texto+=' '+segmentos[i].texto;segmentos[i-1].quebraApos=segmentos[i].quebraApos;segmentos.splice(i,1)}}}return segmentos;}
function audireTtsPausaEntreSegmentos(tipo,pausas){const p=audireTtsPausasLimpa(pausas);if(tipo==='linha_branca')return p.linha_em_branco_segundos;if(tipo==='campo')return p.entre_campos_segundos;if(tipo==='paragrafo')return p.entre_paragrafos_segundos;if(tipo==='respiracao')return p.a_cada_duas_frases_segundos;return .25;}
function audirePcmSilencio(segundos,sampleRate=24000){const amostras=Math.max(0,Math.round(Number(segundos||0)*sampleRate));return Buffer.alloc(amostras*2);}
...
async function audireGoogleTtsGerar({texto,voz,instrucoes,pausas,duracaoSegmentoSegundos}){const key=audireTtsGoogleApiKey();if(!key)throw Object.assign(new Error('Síntese Google não configurada. Defina GEMINI_API_KEY no Render.'),{status:503});const voice=AUDIRE_TTS_GOOGLE_VOZES.includes(String(voz||''))?String(voz):'Kore',model=audireTtsGoogleModelo(),url='https://generativelanguage.googleapis.com/v1beta/interactions',duracao_segmento_segundos=audireTtsDuracaoSegmentoLimpa(duracaoSegmentoSegundos),maxChars=audireTtsMaxCharsPorSegundos(duracao_segmento_segundos),segmentos=audireTtsSegmentarTexto(texto,duracao_segmento_segundos===0?1000000:maxChars);if(!segmentos.length)throw Object.assign(new Error('Não há texto para sintetizar.'),{status:400});const partes=[],mapa=[],interacoes=[];let segundos=0,previousInteractionId='';try{for(let i=0;i<segmentos.length;i++){const seg=segmentos[i],contexto={antes:i>0?segmentos[i-1].texto:'',depois:i+1<segmentos.length?segmentos[i+1].texto:''},gerado=await audireGoogleTtsGerarBloco({texto:seg.texto,voice,model,key,url,instrucoes,pausas,contexto,previousInteractionId}),pcm=gerado.pcm;if(gerado.interaction_id){previousInteractionId=gerado.interaction_id;interacoes.push(gerado.interaction_id)}partes.push(pcm);segundos+=pcm.length/(24000*2);if(i<segmentos.length-1){const pausa=audireTtsPausaEntreSegmentos(seg.quebraApos,pausas);mapa.push({indice:i+1,tipo:seg.quebraApos,posicao_segundos:Number(segundos.toFixed(4)),pausa_segundos:Number(pausa.toFixed(2))});const silencio=audirePcmSilencio(pausa,24000);partes.push(silencio);segundos+=pausa}}const wav=audireWavDePcm(Buffer.concat(partes),24000,1,16);if(wav.length>AUDIRE_AUDIO_MAX_BYTES)throw Object.assign(new Error('A síntese gerada ultrapassa o limite de 20 MB do Audire. Divida o roteiro ou aumente o limite antes de publicar.'),{status:413});return {wav,model,voz:voice,instrucoes:limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,segmentos:mapa,quantidade_segmentos:segmentos.length,duracao_segmento_segundos,max_chars_segmento:maxChars}}finally{if(interacoes.length)audireGoogleTtsApagarInteracoes(interacoes,key,url).catch(()=>{})}}
