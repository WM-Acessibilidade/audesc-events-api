...
function audireTtsPausasLimpa(v){const b=v&&typeof v==='object'?v:{},num=(x,d)=>Number.isFinite(Number(x))?Math.max(0,Math.min(5,Number(x))):d;return {apos_titulo_segundos:num(b.apos_titulo_segundos,1.5),entre_paragrafos_segundos:num(b.entre_paragrafos_segundos,1.5),entre_campos_segundos:num(b.entre_campos_segundos,1.5),a_cada_duas_frases_segundos:num(b.a_cada_duas_frases_segundos,1.0)};}
...
function audireTtsComporTexto(r,cfg,partes,pausas){const p=audireTtsPartesLimpa(partes),ps=audireTtsPausasLimpa(pausas),blocos=[];const add=(tipo,v)=>{const t=String(v||'').replace(/\r\n?/g,'\n').trim();if(t)blocos.push({tipo,texto:t})};if(p.titulo)add('titulo',r.titulo);if(p.roteiro)add('roteiro',r.roteiro_texto);if(p.creditos)add('creditos',r.creditos);if(p.lema)add('lema',cfg?.lema);let texto='';for(let i=0;i<blocos.length;i++){const b=blocos[i];if(i){const anterior=blocos[i-1];texto+=anterior.tipo==='titulo'?'\n\n\n\n\n':'\n\n\n\n';}texto+=b.texto;}return {partes:{...p,pausas:ps},pausas:ps,texto};}
function audireTtsPrompt(texto,instrucoes,pausas,contexto={}){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,antes=limit(contexto?.antes,700),depois=limit(contexto?.depois,700);let ctx='';if(antes||depois){ctx='\n\nContexto do roteiro para preservar sentido e pronúncia. Este contexto NÃO deve ser sintetizado:'+(antes?`\n[antes] ${antes}`:'')+(depois?`\n[depois] ${depois}`:'');}return `Gere áudio de fala para uma audiodescrição em português brasileiro. Sintetize exclusivamente o conteúdo identificado como TRANSCRIÇÃO A FALAR. Não leia instruções nem o contexto. Mantenha as palavras da transcrição na ordem apresentada, sem acrescentar conteúdo.\n\nEstilo de leitura: ${instr}${ctx}\n\nTRANSCRIÇÃO A FALAR:\n${texto}`;}
...
function audireTtsSegmentarTexto(texto,maxChars=650){const fonte=String(texto||'').replace(/\r\n?/g,'\n').trim();if(!fonte)return[];const partes=fonte.split(/(\n{2,})/),unidades=[];for(let i=0;i<partes.length;i+=2){const par=String(partes[i]||'').trim(),sep=String(partes[i+1]||'');if(!par)continue;const frases=par.split(/(?<=[.!?…])\s+(?=[A-ZÁÀÂÃÉÊÍÓÔÕÚÜÇ0-9“"'])/u).map(x=>x.trim()).filter(Boolean),itens=frases.length?frases:[par];for(let j=0;j<itens.length;j++){const pedacos=audireTtsQuebrarUnidadeLonga(itens[j],maxChars);for(let k=0;k<pedacos.length;k++){const fimFrase=k===pedacos.length-1,fimPar=fimFrase&&j===itens.length-1;let quebra='frase';if(fimPar)quebra=sep.length>=5?'titulo':sep.length>=4?'campo':sep.length>=2?'paragrafo':'frase';unidades.push({texto:pedacos[k],quebraApos:quebra,fimFrase});}}}const segmentos=[];let atual='',frasesNoAtual=0,quebra='frase';const minimo=Math.max(140,Math.round(maxChars*.60));const fechar=(tipo)=>{if(atual.trim())segmentos.push({texto:atual.trim(),quebraApos:tipo||quebra});atual='';frasesNoAtual=0;quebra='frase'};for(const u of unidades){const teste=atual?(atual+' '+u.texto):u.texto;if(atual&&teste.length>maxChars)fechar(frasesNoAtual>=2?'respiracao':quebra);atual=atual?(atual+' '+u.texto):u.texto;if(u.fimFrase)frasesNoAtual++;quebra=u.quebraApos;if(['titulo','campo'].includes(u.quebraApos)&&atual.length>=minimo){fechar(u.quebraApos);continue}if(u.quebraApos==='paragrafo'&&atual.length>=minimo){fechar('paragrafo');continue}if(frasesNoAtual>=2&&atual.length>=minimo)fechar('respiracao')}fechar(quebra);if(segmentos.length>1){for(let i=segmentos.length-1;i>0;i--){if(segmentos[i].texto.length<110&&(segmentos[i-1].texto.length+1+segmentos[i].texto.length)<=Math.round(maxChars*1.35)){segmentos[i-1].texto+=' '+segmentos[i].texto;segmentos[i-1].quebraApos=segmentos[i].quebraApos;segmentos.splice(i,1)}}}return segmentos;}
function audireTtsPausaEntreSegmentos(tipo,pausas){const p=audireTtsPausasLimpa(pausas);if(tipo==='titulo')return p.apos_titulo_segundos;if(tipo==='campo')return p.entre_campos_segundos;if(tipo==='paragrafo')return p.entre_paragrafos_segundos;if(tipo==='respiracao')return p.a_cada_duas_frases_segundos;return .25;}
