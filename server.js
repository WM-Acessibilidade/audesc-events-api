...
const AUDIRE_TTS_GOOGLE_MODELOS=['gemini-3.1-flash-tts-preview','gemini-2.5-flash-preview-tts'];
function audireTtsGoogleModelo(){const m=limit(process.env.AUDIRE_TTS_GOOGLE_MODEL,120)||'gemini-3.1-flash-tts-preview';return AUDIRE_TTS_GOOGLE_MODELOS.includes(m)?m:'gemini-3.1-flash-tts-preview';}
function audireTtsGoogleModeloLimpo(v){return AUDIRE_TTS_GOOGLE_MODELOS.includes(String(v||''))?String(v):audireTtsGoogleModelo();}
...
app.get('/audire/admin/tts/config',async(req,res)=>{if(!admin(req,res))return;try{const cfg=await audireObterConfig();res.json({ok:true,provedor:'google-gemini-tts',configurado:!!audireTtsGoogleApiKey(),modelo:audireTtsGoogleModelo(),modelos:AUDIRE_TTS_GOOGLE_MODELOS,vozes:AUDIRE_TTS_GOOGLE_VOZES,padrao:cfg.tts_padrao||audireTtsPadraoLimpo({}),publicacao:cfg.audio_publicacao||audireAudioPublicacaoLimpa({})})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar configuração de síntese.'})}});
