...
function audireSanitizarRoteiro(b){const formatoPub=['mp3','opus'].includes(String(b.audio_publicacao_formato||'').toLowerCase())?String(b.audio_publicacao_formato).toLowerCase():null,bitratePub=formatoPub?audireAudioFormatoLimpo(formatoPub,b.audio_publicacao_bitrate_kbps).bitrate_kbps:null;return {titulo:limit(b.titulo,220),roteiro_texto:limit(b.roteiro_texto,50000),creditos:limit(b.creditos,12000),ordem:Number.isFinite(Number(b.ordem))?Math.trunc(Number(b.ordem)):0,status_publicacao:['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'publicado',categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,nome_local:limit(b.nome_local,220)||null,endereco:limit(b.endereco,500)||null,links_adicionais:audireSanitizarLinks(b.links_adicionais),reproducao_automatica_audio:b.reproducao_automatica_audio===true||String(b.reproducao_automatica_audio)==='true',audio_publicacao_formato:formatoPub,audio_publicacao_bitrate_kbps:bitratePub};}
...
app.post('/audire/admin/roteiros',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRoteiro(req.body||{});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro da audiodescrição.'});const repoId=limit(req.body?.repositorio_id,80)||null;if(repoId){const {data:repo}=await getSupabase().from('audire_repositorios').select('id').eq('id',repoId).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'})}const agora=new Date().toISOString();const {data,error}=await getSupabase().from('audire_roteiros').insert({...b,repositorio_id:repoId,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar roteiro.'})}});
...
app.get('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo,error}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:roteiros,error:er}=await sb.from('audire_roteiros').select('*').eq('repositorio_id',repo.id).order('ordem',{ascending:true}).order('criado_em',{ascending:true});if(er)throw er;const {data:qrs,error:eq}=await sb.from('audire_qr_codes').select('*').eq('repositorio_id',repo.id).order('criado_em',{ascending:false});if(eq)throw eq;res.json({ok:true,repositorio:{...repo,roteiros:roteiros||[],qr_codes:qrs||[]},config:await audireObterConfig()})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositório.'})}});
...
app.delete('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),manter=String(req.query.manter_roteiros||'').toLowerCase()==='true';const {data:repo,error:er}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:rots,error:ert}=await sb.from('audire_roteiros').select('*').eq('repositorio_id',req.params.id);if(ert)throw ert;if(manter){for(const r of rots||[]){const links=(Array.isArray(r.links_adicionais)&&r.links_adicionais.length)?r.links_adicionais:(Array.isArray(repo.links_adicionais)?repo.links_adicionais.filter(x=>['roteiros','ambos'].includes(x.exibir_em||'ambos')):[]);const upd={repositorio_id:null,categoria:r.categoria&&r.categoria!=='outros'?r.categoria:(repo.categoria||'outros'),pais:r.pais||repo.pais||null,pais_codigo:r.pais_codigo||repo.pais_codigo||null,unidade_administrativa:r.unidade_administrativa||repo.unidade_administrativa||null,unidade_codigo:r.unidade_codigo||repo.unidade_codigo||null,nome_local:r.nome_local||repo.nome_local||null,endereco:r.endereco||repo.endereco||null,links_adicionais:links,reproducao_automatica_audio:r.reproducao_automatica_audio||!!repo.reproducao_automatica_audio,atualizado_em:new Date().toISOString()};const q=await sb.from('audire_roteiros').update(upd).eq('id',r.id);if(q.error)throw q.error}await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo.imagem_path);const {error}=await sb.from('audire_repositorios').delete().eq('id',req.params.id);if(error)throw error;return res.json({ok:true,roteiros_preservados:(rots||[]).length})}await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo.imagem_path);for(const r of rots||[]){await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path)}const {error}=await sb.from('audire_repositorios').delete().eq('id',req.params.id);if(error)throw error;res.json({ok:true,roteiros_excluidos:(rots||[]).length})}catch(e){res.status(500).json({error:e.message||'Erro ao excluir repositório.'})}});
...
app.post('/audire/admin/repositorios/:id/vincular-roteiro',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),roteiroId=limit(req.body?.roteiro_id,80);if(!roteiroId)return res.status(400).json({error:'Informe o roteiro avulso.'});const {data:repo}=await sb.from('audire_repositorios').select('id').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:r}=await sb.from('audire_roteiros').select('id,repositorio_id').eq('id',roteiroId).maybeSingle();if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});if(r.repositorio_id)return res.status(409).json({error:'Este roteiro já está vinculado a um repositório.'});const {data,error}=await sb.from('audire_roteiros').update({repositorio_id:repo.id,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao vincular roteiro.'})}});
app.post('/audire/admin/repositorios/:id/roteiros',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRoteiro(req.body||{});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro da audiodescrição.'});const sb=getSupabase();const {data:repo}=await sb.from('audire_repositorios').select('id').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const agora=new Date().toISOString();const {data,error}=await sb.from('audire_roteiros').insert({...b,repositorio_id:repo.id,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar roteiro.'})}});
app.patch('/audire/admin/roteiros/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:antes,error:ea}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(ea)throw ea;if(!antes)return res.status(404).json({error:'Roteiro não encontrado.'});const b=audireSanitizarRoteiro({...antes,...(req.body||{})});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro.'});let repositorio_id=antes.repositorio_id;if(Object.prototype.hasOwnProperty.call(req.body||{},'repositorio_id'))repositorio_id=limit(req.body?.repositorio_id,80)||null;if(repositorio_id){const {data:repo}=await sb.from('audire_repositorios').select('id').eq('id',repositorio_id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'})}else if(antes.repositorio_id){const {data:repoAntigo}=await sb.from('audire_repositorios').select('categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,links_adicionais,reproducao_automatica_audio').eq('id',antes.repositorio_id).maybeSingle();if(repoAntigo){if(!b.pais)b.pais=repoAntigo.pais||null;if(!b.pais_codigo)b.pais_codigo=repoAntigo.pais_codigo||null;if(!b.unidade_administrativa)b.unidade_administrativa=repoAntigo.unidade_administrativa||null;if(!b.unidade_codigo)b.unidade_codigo=repoAntigo.unidade_codigo||null;if(!b.nome_local)b.nome_local=repoAntigo.nome_local||null;if(!b.endereco)b.endereco=repoAntigo.endereco||null;if(!b.categoria||b.categoria==='outros')b.categoria=repoAntigo.categoria||b.categoria;if((!Array.isArray(b.links_adicionais)||!b.links_adicionais.length)&&Array.isArray(repoAntigo.links_adicionais))b.links_adicionais=repoAntigo.links_adicionais.filter(x=>['roteiros','ambos'].includes(x.exibir_em||'ambos'));if(!b.reproducao_automatica_audio)b.reproducao_automatica_audio=!!repoAntigo.reproducao_automatica_audio}}const {data,error}=await sb.from('audire_roteiros').update({...b,repositorio_id,atualizado_em:new Date().toISOString()}).eq('id',req.params.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao atualizar roteiro.'})}});
