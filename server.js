...
function audireSanitizarRepositorio(b){const status=['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'rascunho';const raio=Math.max(10,Math.min(5000,Math.round(Number(b.gps_raio_metros)||200))),prec=Math.max(10,Math.min(5000,Math.round(Number(b.gps_precisao_max_metros)||500)));return {titulo:limit(b.titulo,220),descricao:limit(b.descricao,12000),categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,nome_local:limit(b.nome_local,220)||null,endereco:limit(b.endereco,500)||null,latitude:Number.isFinite(Number(b.latitude))?Number(b.latitude):null,longitude:Number.isFinite(Number(b.longitude))?Number(b.longitude):null,google_place_id:limit(b.google_place_id,220)||null,imagem_alt:limit(b.imagem_alt,500)||null,status_publicacao:status,exigir_presenca_roteiros:b.exigir_presenca_roteiros===true||String(b.exigir_presenca_roteiros)==='true',gps_raio_metros:raio,gps_precisao_max_metros:prec};}
