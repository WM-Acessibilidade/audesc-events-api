...
app.post('/audire/admin/roteiros/:id/tts/preview',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),{data:r,error}=await sb.from('audire_roteiros').select('id,titulo,roteiro_texto,creditos').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),fonte={titulo:Object.prototype.hasOwnProperty.call(req.body||{},'titulo')?limit(req.body.titulo,220):r.titulo,roteiro_texto:Object.prototype.hasOwnProperty.call(req.body||{},'roteiro_texto')?limit(req.body.roteiro_texto,50000):r.roteiro_texto,creditos:Object.prototype.hasOwnProperty.call(req.body||{},'creditos')?limit(req.body.creditos,12000):r.creditos},comp=audireTtsComporTexto(fonte,cfg,req.body?.partes||cfg.tts_padrao?.partes,req.body?.pausas||cfg.tts_padrao?.pausas);if(!comp.texto.trim())return res.status(400).json({error:'Selecione ao menos uma parte que contenha texto para gerar a síntese.'});const out=await audireGoogleTtsGerar({texto:comp.texto,voz:req.body?.voz||cfg.tts_padrao?.voz,instrucoes:req.body?.instrucoes||cfg.tts_padrao?.instrucoes,pausas:comp.pausas,duracaoSegmentoSegundos:req.body?.duracao_segmento_segundos,normalizacao:req.body?.normalizacao??cfg.tts_padrao?.normalizacao,campos:comp.blocos?.map(b=>b.tipo)||[]});res.set({'Content-Type':'audio/wav','Content-Length':String(out.wav.length),'Cache-Control':'no-store','X-Audire-TTS-Modelo':out.model,'X-Audire-TTS-Voz':out.voz,'X-Audire-TTS-Partes':encodeURIComponent(JSON.stringify(comp.partes)),'X-Audire-TTS-Segmentos':encodeURIComponent(JSON.stringify(out.segmentos||[])),'X-Audire-TTS-Quantidade-Segmentos':String(out.quantidade_segmentos||1),'X-Audire-TTS-Duracao-Segmento':String(out.duracao_segmento_segundos===0?0:(out.duracao_segmento_segundos||30)),'X-Audire-TTS-Normalizacao':out.normalizacao||'moderada','X-Audire-TTS-Revisao':out.revisao_id||'','Access-Control-Expose-Headers':'X-Audire-TTS-Modelo, X-Audire-TTS-Voz, X-Audire-TTS-Partes, X-Audire-TTS-Segmentos, X-Audire-TTS-Quantidade-Segmentos, X-Audire-TTS-Duracao-Segmento, X-Audire-TTS-Normalizacao, X-Audire-TTS-Revisao'}).send(out.wav)}catch(e){const status=Number(e.status)||500;res.status(status).json({error:e.message||'Erro ao gerar prévia de síntese.'})}});
