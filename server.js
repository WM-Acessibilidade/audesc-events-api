...
function audireConfigLimpa(c){const b=c&&typeof c==='object'?c:{};return {paises_habilitados:Array.isArray(b.paises_habilitados)?[...new Set(b.paises_habilitados.map(x=>String(x||'').trim().toUpperCase()).filter(Boolean))]:null,lema:limit(b.lema,300)||'Audire. Para que todos possam ouvir o que dizem as imagens.'};}
...
app.post('/audire/admin/roteiros/:id/audio',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeAudioValido(mime))return res.status(400).json({error:'Formato de áudio não suportado. Use MP3, WAV, OGG, WebM, M4A ou AAC.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio inválido ou maior que 20 MB.'});const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/audios/${r.id}-${Date.now()}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const {data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar áudio do roteiro.'})}});
