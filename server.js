...
function audireSanitizarRoteiro(b){const formatoPub=['mp3','opus'].includes(String(b.audio_publicacao_formato||'').toLowerCase())?String(b.audio_publicacao_formato).toLowerCase():null,bitratePub=formatoPub?audireAudioFormatoLimpo(formatoPub,b.audio_publicacao_bitrate_kbps).bitrate_kbps:null;return {titulo:limit(b.titulo,220),roteiro_texto:limit(b.roteiro_texto,50000),creditos:limit(b.creditos,12000),ordem:Number.isFinite(Number(b.ordem))?Math.trunc(Number(b.ordem)):0,status_publicacao:['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'publicado',categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,links_adicionais:audireSanitizarLinks(b.links_adicionais),reproducao_automatica_audio:b.reproducao_automatica_audio===true||String(b.reproducao_automatica_audio)==='true',audio_publicacao_formato:formatoPub,audio_publicacao_bitrate_kbps:bitratePub};}
...
app.delete('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo}=await sb.from('audire_repositorios').select('imagem_path').eq('id',req.params.id).maybeSingle();const {data:rots}=await sb.from('audire_roteiros').select('imagem_path,audio_path').eq('repositorio_id',req.params.id);await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo?.imagem_path);for(const r of rots||[]){await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path)}const {error}=await sb.from('audire_repositorios').delete().eq('id',req.params.id);if(error)throw error;res.json({ok:true})}catch(e){res.status(500).json({error:e.message||'Erro ao excluir repositório.'})}});
