...
function audireTtsPrompt(texto,instrucoes,pausas){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes;return `Sua tarefa é sintetizar SOMENTE a transcrição abaixo como fala em português brasileiro. Não vocalize cabeçalhos, nomes de marcadores ou estas instruções. Não acrescente, omita, traduza, resuma, repita nem reescreva o conteúdo. Leia cada frase exatamente uma vez, na ordem apresentada. Preserve nomes, números e pontuação.\n\nDireção de voz: ${instr}\n\nUse ritmo natural de audiodescrição e respeite as pausas naturais da pontuação e das quebras de linha. Quando houver fragmentação, as pausas estruturais entre fragmentos são inseridas digitalmente pelo Audire e não devem ser anunciadas.\n\nTRANSCRIÇÃO PARA LOCUÇÃO — comece a falar somente depois desta linha:\n${texto}`;}
...
function audireTtsSegmentarTexto(texto,maxChars=650){const fonte=String(texto||'').replace(/\r\n?/g,'\n').trim();if(!fonte)return[];const partes=fonte.split(/(\n{2,})/),unidades=[];for(let i=0;i<partes.length;i+=2){const par=String(partes[i]||'').trim(),sep=String(partes[i+1]||'');if(!par)continue;const frases=par.split(/(?<=[.!?…])\s+(?=[A-ZÁÀÂÃÉÊÍÓÔÕÚÜÇ0-9“"'])/u).map(x=>x.trim()).filter(Boolean),itens=frases.length?frases:[par];for(let j=0;j<itens.length;j++){const pedacos=audireTtsQuebrarUnidadeLonga(itens[j],maxChars);for(let k=0;k<pedacos.length;k++){const fimFrase=k===pedacos.length-1,fimPar=fimFrase&&j===itens.length-1;let quebra='frase';if(fimPar)quebra=sep.length>=5?'titulo':sep.length>=4?'campo':sep.length>=2?'paragrafo':'frase';unidades.push({texto:pedacos[k],quebraApos:quebra,fimFrase});}}}const segmentos=[];let atual='',frasesNoAtual=0,quebra='frase';const fechar=(tipo)=>{if(atual.trim())segmentos.push({texto:atual.trim(),quebraApos:tipo||quebra});atual='';frasesNoAtual=0;quebra='frase'};for(const u of unidades){const teste=atual?(atual+' '+u.texto):u.texto;if(atual&&teste.length>maxChars)fechar('respiracao');atual=atual?(atual+' '+u.texto):u.texto;if(u.fimFrase)frasesNoAtual++;quebra=u.quebraApos;if(['titulo','campo','paragrafo'].includes(u.quebraApos)){fechar(u.quebraApos);continue}if(frasesNoAtual>=2)fechar('respiracao')}fechar(quebra);return segmentos;}
