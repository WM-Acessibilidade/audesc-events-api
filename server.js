...
const AVALIACAO_PADRAO = {ativa:true,prazo_minutos:60,participacao_minima_minutos:1,permitir_edicao:false,comentario_ativo:true,itens_avaliacao:ITENS_AVALIACAO_PADRAO};
async function obterConfigAvaliacao(){
  try{
    ...
    if(error) throw error;
    const cfg=Object.assign({},AVALIACAO_PADRAO,data||{});
    cfg.itens_avaliacao=Object.assign({},ITENS_AVALIACAO_PADRAO,(data&&data.itens_avaliacao)||{});
    return cfg;
  }catch(e){ return Object.assign({},AVALIACAO_PADRAO,{itens_avaliacao:Object.assign({},ITENS_AVALIACAO_PADRAO)}); }
}
...
app.post('/public/salas/:sala/avaliacao', async (req,res)=>{
 try{
  ...
  const cfg=await obterConfigAvaliacao(), itens=cfg.itens_avaliacao||ITENS_AVALIACAO_PADRAO;
  const valor5=(chaveCampo)=>{
    if(!itens[chaveCampo]?.exibir) return null;
    const bruto=b[chaveCampo];
    if((bruto===''||bruto==null) && !itens[chaveCampo]?.obrigatorio) return null;
    const n=Number(bruto);
    if(!Number.isInteger(n)||n<1||n>5) throw Object.assign(new Error('Preencha corretamente os itens obrigatórios da avaliação.'),{statusCode:400});
    return n;
  };
  let geral,ad,audio,autonomia,rec=null,comentario='';
  try{
    geral=valor5('avaliacao_geral'); ad=valor5('qualidade_audiodescricao'); audio=valor5('qualidade_audio'); autonomia=valor5('autonomia');
    if(itens.recomendacao?.exibir){
      if((b.recomendacao===''||b.recomendacao==null) && itens.recomendacao.obrigatorio) throw Object.assign(new Error('Preencha a recomendação.'),{statusCode:400});
      if(b.recomendacao!==''&&b.recomendacao!=null){rec=Number(b.recomendacao);if(!Number.isInteger(rec)||rec<0||rec>10)throw Object.assign(new Error('A recomendação deve estar entre 0 e 10.'),{statusCode:400});}
    }
    if(itens.comentario?.exibir){
      comentario=limit(b.comentario,1000);
      if(itens.comentario.obrigatorio && !comentario) throw Object.assign(new Error('Preencha o campo de comentários e sugestões.'),{statusCode:400});
    }
  }catch(validacao){return res.status(validacao.statusCode||400).json({error:validacao.message});}
  const payload={evento_id:el.evento_id,sala_codigo:sala,ouvinte_id:chave,avaliacao_geral:geral,qualidade_audiodescricao:ad,qualidade_audio:audio,autonomia,recomendacao:rec,comentario,atualizada_em:new Date().toISOString()};
...
app.put('/admin/avaliacoes/configuracao', async (req,res)=>{
 if(!admin(req,res))return;
 try{
  const b=req.body||{}, recebidos=b.itens_avaliacao||{}, itens={};
  Object.keys(ITENS_AVALIACAO_PADRAO).forEach(chave=>{
    const r=recebidos[chave]||{}, padrao=ITENS_AVALIACAO_PADRAO[chave];
    itens[chave]={exibir:r.exibir!==false,obrigatorio:r.obrigatorio===true};
    if(!itens[chave].exibir) itens[chave].obrigatorio=false;
    if(r.exibir===undefined) itens[chave].exibir=padrao.exibir;
    if(r.obrigatorio===undefined) itens[chave].obrigatorio=padrao.obrigatorio;
  });
  const payload={
    id:1,ativa:b.ativa!==false,prazo_minutos:Math.max(15,Math.min(120,Number(b.prazo_minutos||60))),
    participacao_minima_minutos:Math.max(0,Math.min(60,Number(b.participacao_minima_minutos||1))),
    permitir_edicao:false,comentario_ativo:itens.comentario.exibir,
    avaliacao_instantanea_ativa:b.avaliacao_instantanea_ativa!==false,
    comentario_instantaneo_ativo:b.comentario_instantaneo_ativo!==false,
    itens_avaliacao:itens,updated_at:new Date().toISOString()
  };
  const {error}=await getSupabase().from('configuracao_avaliacoes').upsert(payload);if(error)throw error;
...
app.get('/public/avaliacoes/configuracao', async (req,res)=>{
  try{
    const cfgInstantanea=await avaliacaoInstantaneaAtiva();
    const cfgFinal=await obterConfigAvaliacao();
    res.set('Cache-Control','no-store');
    res.json({
      ok:true,
      avaliacao_instantanea_ativa:cfgInstantanea.ativa,
      comentario_instantaneo_ativo:cfgInstantanea.comentario,
      avaliacao_final_ativa:cfgFinal.ativa!==false,
      itens_avaliacao:cfgFinal.itens_avaliacao||ITENS_AVALIACAO_PADRAO
    });
  }catch(e){
...
app.post('/salas/:sala/avaliacao-instantanea/pedidos', async (req,res)=>{
 try{
  ...
  // Um novo pedido encerra o ciclo anterior; o novo permanece disponível ao ouvinte
  // até ser respondido, substituído por outro pedido ou a transmissão terminar.
  await sb.from('pedidos_avaliacao_audiodescricao').update({ativo:false}).eq('evento_id',ev.id).eq('ativo',true);
  const expiraBanco=new Date('2099-12-31T23:59:59.000Z');
  const {data:pedido,error:pe}=await sb.from('pedidos_avaliacao_audiodescricao').insert({evento_id:ev.id,sala_codigo:ev.sala_codigo||sala,solicitado_por:limit(b.solicitado_por,120),criado_em:agora.toISOString(),expira_em:expiraBanco.toISOString(),ativo:true}).select('*').single(); if(pe) throw pe;
  ...
  res.json({ok:true,pedido_id:pedido.id,expira_em:null,proximo_pedido_em:new Date(agora.getTime()+AVALIACAO_INSTANTANEA_INTERVALO_MINUTOS*60000).toISOString(),elegiveis:rows.length,comentario_ativo:cfg.comentario});
...
app.get('/public/salas/:sala/avaliacao-instantanea/status', async (req,res)=>{
 try{
  ...
  for(const el of (els||[])){
    const p=el.pedidos_avaliacao_audiodescricao; if(!p||!p.ativo) continue;
    ...
    if(!av) return res.json({ok:true,ativa:true,disponivel:true,pedido_id:p.id,expira_em:null,mensagem:'Por gentileza, avalie a minha audiodescricao.'});
  }
...
app.post('/public/salas/:sala/avaliacao-instantanea', async (req,res)=>{
 try{
  ...
  const p=el.pedidos_avaliacao_audiodescricao; if(!p||!p.ativo) return res.status(410).json({error:'Este pedido de avaliação não está mais ativo.'});
...
// Fase 6.21.2 — Preferências persistentes de alertas de monitoramento.
const MONITOR_ALERT_CONFIG_ID='monitoramento-alertas';
let MONITOR_ALERT_WATCHER_BASELINE=null;
let MONITOR_ALERT_WATCHER_BUSY=false;
function defaultMonitorAlertConfig(){return {ativo:false,tipos:{evento_publico:true,evento_restrito:true,demonstracao:true},canais:{som:true,navegador:false,email:false},emails:['contato@audesc.com']};}
function emailMonitorValido(v){const s=String(v||'').trim().toLowerCase();return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s)?s:'';}
function sanitizarMonitorAlertConfig(input){const b=input&&typeof input==='object'?input:{},p=defaultMonitorAlertConfig(),emails=Array.isArray(b.emails)?b.emails:[],unicos=[...new Set(emails.map(emailMonitorValido).filter(Boolean))].slice(0,20);return {ativo:b.ativo===true,tipos:{evento_publico:b.tipos?.evento_publico!==false,evento_restrito:b.tipos?.evento_restrito!==false,demonstracao:b.tipos?.demonstracao!==false},canais:{som:b.canais?.som!==false,navegador:b.canais?.navegador===true,email:b.canais?.email===true},emails:unicos.length?unicos:p.emails};}
async function obterMonitorAlertConfig(){const {data,error}=await getSupabase().from('formulario_config').select('config').eq('id',MONITOR_ALERT_CONFIG_ID).maybeSingle();if(error)throw error;return sanitizarMonitorAlertConfig(data&&data.config?data.config:defaultMonitorAlertConfig());}
...
// -----------------------------------------------------------------------------
// Audire Fase 1.2 — domínio isolado de repositórios permanentes de audiodescrição.
// Tabelas próprias audire_*, rotas /audire/* e buckets exclusivos audire-*.
// -----------------------------------------------------------------------------
const AUDIRE_REPO_BUCKET='audire-repositorios';
const AUDIRE_ROTEIRO_BUCKET='audire-roteiros';
const AUDIRE_IMAGE_MAX_BYTES=3*1024*1024;
const AUDIRE_AUDIO_MAX_BYTES=20*1024*1024;
const AUDIRE_PRESENCA_TTL_SEGUNDOS=8*60*60;
function audirePublicarRepositorioSelect(){return 'id,titulo,descricao,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,latitude,longitude,google_place_id,imagem_url,imagem_alt,status_publicacao,publicado_em,criado_em,atualizado_em,exigir_presenca_roteiros,gps_raio_metros,gps_precisao_max_metros,links_adicionais,reproducao_automatica_audio,ordem_destaque,ocultar_voltar_repositorio,ocultar_voltar_roteiro';}
function audireTtsPartesLimpa(p){const b=p&&typeof p==='object'?p:{};return {titulo:b.titulo!==false,roteiro:b.roteiro!==false,creditos:b.creditos===true,lema:b.lema===true};}
function audireTtsPausasLimpa(v){const b=v&&typeof v==='object'?v:{},num=(x,d)=>Number.isFinite(Number(x))?Math.max(0,Math.min(5,Number(x))):d,linha=num(b.linha_em_branco_segundos??b.apos_titulo_segundos,3),campo=num(b.troca_campo_segundos,2),frase=num(b.apos_frase_segundos??b.a_cada_duas_frases_segundos,1);return {linha_em_branco_segundos:linha,troca_campo_segundos:campo,apos_titulo_segundos:linha,apos_frase_segundos:frase};}
function audireTtsNormalizacaoLimpa(v){const x=String(v||'moderada').toLowerCase();return ['desativada','moderada','forte'].includes(x)?x:'moderada';}
function audireTtsPadraoLimpo(v){const b=v&&typeof v==='object'?v:{};return {voz:limit(b.voz,80)||'Kore',instrucoes:limit(b.instrucoes,3000)||'Leitura clara, natural e acolhedora, em português brasileiro. Ritmo moderado, dicção precisa e tom neutro, próprio para audiodescrição. Evite dramatização excessiva.',partes:audireTtsPartesLimpa(b.partes),pausas:audireTtsPausasLimpa(b.pausas),normalizacao:audireTtsNormalizacaoLimpa(b.normalizacao)};}
function audireAudioPublicacaoLimpa(v){const b=v&&typeof v==='object'?v:{},formato=String(b.formato||'mp3').toLowerCase()==='opus'?'opus':'mp3',permitidos=formato==='opus'?[32,48,64,96]:[48,64,96],padrao=formato==='opus'?48:64,bitrate=permitidos.includes(Number(b.bitrate_kbps))?Number(b.bitrate_kbps):padrao;return {formato,bitrate_kbps:bitrate};}
function audireConfigLimpa(c){const b=c&&typeof c==='object'?c:{};return {paises_habilitados:Array.isArray(b.paises_habilitados)?[...new Set(b.paises_habilitados.map(x=>String(x||'').trim().toUpperCase()).filter(Boolean))]:null,lema:limit(b.lema,300)||'Audire. Para que todos possam ouvir o que dizem as imagens.',tts_padrao:audireTtsPadraoLimpo(b.tts_padrao),audio_publicacao:audireAudioPublicacaoLimpa(b.audio_publicacao)};}
async function audireObterConfig(){const {data,error}=await getSupabase().from('audire_configuracao').select('config').eq('id','geral').maybeSingle();if(error)throw error;return audireConfigLimpa(data?.config||{});}
async function audireSalvarConfig(c){const config=audireConfigLimpa(c),{data,error}=await getSupabase().from('audire_configuracao').upsert({id:'geral',config,atualizado_em:new Date().toISOString()},{onConflict:'id'}).select('config').single();if(error)throw error;return audireConfigLimpa(data.config);}
function audirePaisHabilitado(repo,cfg){const a=cfg?.paises_habilitados;if(!Array.isArray(a))return true;return a.includes(String(repo?.pais_codigo||'').toUpperCase());}
function audireSegredoPresenca(){return String(process.env.AUDIRE_PRESENCA_SECRET||ADMIN_TOKEN||'').trim();}
function audireBase64Url(v){return Buffer.from(v).toString('base64url')}
function audireAssinarAcesso(repoId,origem='gps'){const segredo=audireSegredoPresenca();if(!segredo)throw new Error('Configure AUDIRE_PRESENCA_SECRET ou ADMIN_TOKEN para usar controle de presença.');const payload={repo:String(repoId),origem,exp:Math.floor(Date.now()/1000)+AUDIRE_PRESENCA_TTL_SEGUNDOS};const corpo=audireBase64Url(JSON.stringify(payload));const sig=crypto.createHmac('sha256',segredo).update(corpo).digest('base64url');return corpo+'.'+sig;}
function audireValidarAcesso(token,repoId){try{const segredo=audireSegredoPresenca();if(!segredo||!token)return false;const [corpo,sig]=String(token).split('.');if(!corpo||!sig)return false;const esperado=crypto.createHmac('sha256',segredo).update(corpo).digest('base64url');const a=Buffer.from(sig),b=Buffer.from(esperado);if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return false;const p=JSON.parse(Buffer.from(corpo,'base64url').toString('utf8'));return String(p.repo)===String(repoId)&&Number(p.exp)>Math.floor(Date.now()/1000)}catch{return false}}
function audireDistanciaMetros(lat1,lon1,lat2,lon2){const R=6371000,p=Math.PI/180,dLat=(lat2-lat1)*p,dLon=(lon2-lon1)*p,a=Math.sin(dLat/2)**2+Math.cos(lat1*p)*Math.cos(lat2*p)*Math.sin(dLon/2)**2;return 2*R*Math.atan2(Math.sqrt(a),Math.sqrt(1-a));}
async function audireGarantirBucket(nome,limite,mimes){const sb=getSupabase();const {data,error}=await sb.storage.getBucket(nome);if(!error&&data)return;const criado=await sb.storage.createBucket(nome,{public:true,fileSizeLimit:limite,allowedMimeTypes:mimes});if(criado.error&&!String(criado.error.message||'').toLowerCase().includes('already'))throw criado.error;}
function audireMimeImagemValido(m){return ['image/jpeg','image/png','image/webp'].includes(String(m||'').toLowerCase())}
function audireMimeAudioValido(m){return ['audio/mpeg','audio/mp3','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a'].includes(String(m||'').toLowerCase())}
function audireExtMime(m){m=String(m||'').toLowerCase();if(m==='image/png')return'png';if(m==='image/webp')return'webp';if(m.startsWith('image/'))return'jpg';if(m.includes('wav'))return'wav';if(m.includes('ogg'))return'ogg';if(m.includes('webm'))return'webm';if(m.includes('mp4')||m.includes('m4a'))return'm4a';if(m.includes('aac'))return'aac';return'mp3'}
async function audireRemoverArquivo(bucket,path){if(!path)return;try{await getSupabase().storage.from(bucket).remove([path])}catch(e){console.warn('Audire: falha ao remover arquivo antigo:',e.message||e)}}
function audireSanitizarLinks(valor){const a=Array.isArray(valor)?valor:[];return a.slice(0,30).map(x=>{const exibir=['repositorio','roteiros','ambos'].includes(String(x?.exibir_em||''))?String(x.exibir_em):'ambos';return {nome:limit(x?.nome,160),url:limit(x?.url,1000),exibir_em:exibir}}).filter(x=>x.nome&&/^https?:\/\//i.test(x.url));}
function audireSanitizarRepositorio(b){const status=['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'rascunho';const raio=Math.max(10,Math.min(5000,Math.round(Number(b.gps_raio_metros)||200))),prec=Math.max(10,Math.min(5000,Math.round(Number(b.gps_precisao_max_metros)||500)));return {titulo:limit(b.titulo,220),descricao:limit(b.descricao,12000),categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,nome_local:limit(b.nome_local,220)||null,endereco:limit(b.endereco,500)||null,latitude:Number.isFinite(Number(b.latitude))?Number(b.latitude):null,longitude:Number.isFinite(Number(b.longitude))?Number(b.longitude):null,google_place_id:limit(b.google_place_id,220)||null,imagem_alt:limit(b.imagem_alt,500)||null,status_publicacao:status,exigir_presenca_roteiros:b.exigir_presenca_roteiros===true||String(b.exigir_presenca_roteiros)==='true',gps_raio_metros:raio,gps_precisao_max_metros:prec,links_adicionais:audireSanitizarLinks(b.links_adicionais),reproducao_automatica_audio:b.reproducao_automatica_audio===true||String(b.reproducao_automatica_audio)==='true',ordem_destaque:(b.ordem_destaque===null||b.ordem_destaque===undefined||String(b.ordem_destaque).trim()==='')?null:(Number.isFinite(Number(b.ordem_destaque))?Math.max(0,Math.trunc(Number(b.ordem_destaque))):null),ocultar_voltar_repositorio:b.ocultar_voltar_repositorio===true||String(b.ocultar_voltar_repositorio)==='true',ocultar_voltar_roteiro:b.ocultar_voltar_roteiro===true||String(b.ocultar_voltar_roteiro)==='true'};}
function audireSanitizarRoteiro(b){const formatoPub=['mp3','opus'].includes(String(b.audio_publicacao_formato||'').toLowerCase())?String(b.audio_publicacao_formato).toLowerCase():null,bitratePub=formatoPub?audireAudioFormatoLimpo(formatoPub,b.audio_publicacao_bitrate_kbps).bitrate_kbps:null;return {titulo:limit(b.titulo,220),roteiro_texto:limit(b.roteiro_texto,50000),creditos:limit(b.creditos,12000),ordem:Number.isFinite(Number(b.ordem))?Math.max(0,Math.trunc(Number(b.ordem))):0,status_publicacao:['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'publicado',categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,nome_local:limit(b.nome_local,220)||null,endereco:limit(b.endereco,500)||null,links_adicionais:audireSanitizarLinks(b.links_adicionais),reproducao_automatica_audio:b.reproducao_automatica_audio===true||String(b.reproducao_automatica_audio)==='true',audio_publicacao_formato:formatoPub,audio_publicacao_bitrate_kbps:bitratePub};}
function audireLocalChave(valor){return String(valor||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');}
function audireLocalIgual(codigoA,nomeA,codigoB,nomeB){const ca=audireLocalChave(codigoA),cb=audireLocalChave(codigoB);if(ca&&cb)return ca===cb;const na=audireLocalChave(nomeA),nb=audireLocalChave(nomeB);return !!na&&!!nb&&na===nb;}
...
// Integração definitiva Cloud TTS: rotas temporárias de diagnóstico/amostras removidas.

// Audire — provedor Google Cloud TTS / Chirp 3 HD.
const AUDIRE_TTS_CHIRP_VOZES=['Achernar','Achird','Algenib','Algieba','Alnilam','Aoede','Autonoe','Callirrhoe','Charon','Despina','Enceladus','Erinome','Fenrir','Gacrux','Iapetus','Kore','Laomedeia','Leda','Orus','Puck','Pulcherrima','Rasalgethi','Sadachbia','Sadaltager','Schedar','Sulafat','Umbriel','Vindemiatrix','Zephyr','Zubenelgenubi'];
function audireTtsProvedorLimpo(v){return String(v||'gemini').toLowerCase()==='chirp3-hd'?'chirp3-hd':'gemini';}
function audireTtsChirpNomeVoz(v){const x=String(v||'Kore').replace(/^pt-BR-Chirp3-HD-/i,'');return AUDIRE_TTS_CHIRP_VOZES.includes(x)?x:'Kore';}
function audireTtsChirpModelo(){return 'Chirp 3 HD';}
async function audireChirpTtsGerarBloco({texto,voice}){
  const cliente=await audireCloudTtsCliente(),nome='pt-BR-Chirp3-HD-'+audireTtsChirpNomeVoz(voice);
  try{
    const r=await cliente.request({url:'https://texttospeech.googleapis.com/v1/text:synthesize',method:'POST',data:{input:{text:String(texto||'')},voice:{languageCode:'pt-BR',name:nome},audioConfig:{audioEncoding:'LINEAR16',sampleRateHertz:24000}}});
    const wav=Buffer.from(String(r.data?.audioContent||''),'base64');
    if(!wav.length)throw Object.assign(new Error('O Chirp 3 HD retornou áudio vazio.'),{status:502});
    const lido=audireLerWavPcm16(wav);
    if(lido.sampleRate!==24000)throw Object.assign(new Error(`O Chirp 3 HD retornou áudio em ${lido.sampleRate} Hz; o Audire esperava 24000 Hz.`),{status:502});
    let pcm=lido.data;
    if(lido.channels===2){const mono=Buffer.alloc(Math.floor(pcm.length/4)*2);for(let i=0,j=0;i+3<pcm.length;i+=4,j+=2){const a=pcm.readInt16LE(i),b=pcm.readInt16LE(i+2);mono.writeInt16LE(Math.max(-32768,Math.min(32767,Math.round((a+b)/2))),j)}pcm=mono}
    return {pcm};
  }catch(e){const msg=e?.response?.data?.error?.message||e.message||'Falha no Chirp 3 HD.';throw Object.assign(new Error(msg),{status:Number(e?.response?.status||e.status)||500});}
}
...
function audireTtsGoogleApiKey(){return String(process.env.GEMINI_API_KEY||process.env.GOOGLE_API_KEY||'').trim();}
function audirePcmSilencio(segundos,sampleRate=24000,channels=1,bits=16){const duracao=Math.max(0,Number(segundos)||0),bytesPorAmostra=Math.max(1,Math.floor(bits/8)),amostras=Math.max(0,Math.round(duracao*sampleRate));return Buffer.alloc(amostras*channels*bytesPorAmostra);}
function audireWavDePcm(pcm,sampleRate=24000,channels=1,bits=16){const data=Buffer.isBuffer(pcm)?pcm:Buffer.from(pcm);const h=Buffer.alloc(44),byteRate=sampleRate*channels*bits/8,blockAlign=channels*bits/8;h.write('RIFF',0);h.writeUInt32LE(36+data.length,4);h.write('WAVE',8);h.write('fmt ',12);h.writeUInt32LE(16,16);h.writeUInt16LE(1,20);h.writeUInt16LE(channels,22);h.writeUInt32LE(sampleRate,24);h.writeUInt32LE(byteRate,28);h.writeUInt16LE(blockAlign,32);h.writeUInt16LE(bits,34);h.write('data',36);h.writeUInt32LE(data.length,40);return Buffer.concat([h,data]);}
function audireLerWavPcm16(wav){const b=Buffer.isBuffer(wav)?wav:Buffer.from(wav||[]);if(b.length<44||b.toString('ascii',0,4)!=='RIFF'||b.toString('ascii',8,12)!=='WAVE')throw Object.assign(new Error('O arquivo recebido não é um WAV válido.'),{status:400});let off=12,fmt=null,data=null;while(off+8<=b.length){const id=b.toString('ascii',off,off+4),len=b.readUInt32LE(off+4),ini=off+8,fim=ini+len;if(fim>b.length)break;if(id==='fmt '){fmt={audioFormat:b.readUInt16LE(ini),channels:b.readUInt16LE(ini+2),sampleRate:b.readUInt32LE(ini+4),bits:b.readUInt16LE(ini+14)}}else if(id==='data'){data=b.subarray(ini,fim)}off=fim+(len%2)}if(!fmt||!data||fmt.audioFormat!==1||fmt.bits!==16||![1,2].includes(fmt.channels))throw Object.assign(new Error('Para converter em MP3, o Audire aceita WAV PCM de 16 bits mono ou estéreo.'),{status:400});return {...fmt,data};}
function audireAudioFormatoLimpo(formato,bitrate){const f=String(formato||'mp3').toLowerCase()==='opus'?'opus':'mp3',permitidos=f==='opus'?[32,48,64,96]:[48,64,96],padrao=f==='opus'?48:64,kbps=permitidos.includes(Number(bitrate))?Number(bitrate):padrao;return {formato:f,bitrate_kbps:kbps,ext:f==='opus'?'ogg':'mp3',mime:f==='opus'?'audio/ogg':'audio/mpeg'};}
function audireFfmpegArgsAudio(formato,bitrate){const a=audireAudioFormatoLimpo(formato,bitrate),base=['-hide_banner','-loglevel','error','-i','pipe:0','-vn'];if(a.formato==='opus')return {...a,args:[...base,'-c:a','libopus','-b:a',`${a.bitrate_kbps}k`,'-vbr','on','-application','audio','-f','ogg','pipe:1']};return {...a,args:[...base,'-c:a','libmp3lame','-b:a',`${a.bitrate_kbps}k`,'-f','mp3','pipe:1']};}
function audireFfmpegConverterAudio(wav,formato='mp3',bitrate=64){const entrada=Buffer.isBuffer(wav)?wav:Buffer.from(wav||[]);if(!entrada.length)throw Object.assign(new Error('Áudio WAV vazio.'),{status:400});const cfg=audireFfmpegArgsAudio(formato,bitrate);return new Promise((resolve,reject)=>{let proc;try{proc=spawn('ffmpeg',cfg.args,{stdio:['pipe','pipe','pipe'],windowsHide:true})}catch(e){return reject(Object.assign(new Error('FFmpeg não pôde ser iniciado no servidor. Verifique se o binário ffmpeg está disponível no Render.'),{status:500,cause:e}))}const saida=[],erros=[];let outBytes=0,errBytes=0,finalizado=false;const timer=setTimeout(()=>{if(!finalizado){proc.kill('SIGKILL');finalizado=true;reject(Object.assign(new Error('A conversão de áudio excedeu 90 segundos e foi interrompida.'),{status:504}))}},90000);proc.stdout.on('data',c=>{outBytes+=c.length;if(outBytes>AUDIRE_AUDIO_MAX_BYTES*2){proc.kill('SIGKILL');return}saida.push(c)});proc.stderr.on('data',c=>{if(errBytes<12000){erros.push(c);errBytes+=c.length}});proc.on('error',e=>{if(finalizado)return;finalizado=true;clearTimeout(timer);const msg=e?.code==='ENOENT'?'FFmpeg não foi encontrado no servidor Render.':'Falha ao iniciar o FFmpeg: '+(e.message||e);reject(Object.assign(new Error(msg),{status:500,cause:e}))});proc.on('close',code=>{if(finalizado)return;finalizado=true;clearTimeout(timer);const stderr=Buffer.concat(erros).toString('utf8').trim();if(code!==0)return reject(Object.assign(new Error('FFmpeg não conseguiu converter o áudio'+(stderr?': '+stderr.slice(0,800):'.')),{status:500}));const buffer=Buffer.concat(saida);if(!buffer.length)return reject(Object.assign(new Error('FFmpeg concluiu sem produzir arquivo de áudio.'),{status:500}));resolve({...cfg,buffer,tamanho_bytes:buffer.length})});proc.stdin.on('error',()=>{});proc.stdin.end(entrada);});}
async function audireWavParaMp3(wav,bitrate=64){const out=await audireFfmpegConverterAudio(wav,'mp3',bitrate);return {mp3:out.buffer,bitrate_kbps:out.bitrate_kbps};}
const AUDIRE_TTS_MARCADOR_CAMPO='\uE000AUDIRE_CAMPO\uE001';
function audireTtsComporTexto(r,cfg,partes,pausas){const p=audireTtsPartesLimpa(partes),ps=audireTtsPausasLimpa(pausas),blocos=[];const add=(tipo,v)=>{const t=String(v||'').replace(/\r\n?/g,'\n').trim();if(t)blocos.push({tipo,texto:t})};if(p.titulo)add('titulo',r.titulo);if(p.roteiro)add('roteiro',r.roteiro_texto);if(p.creditos)add('creditos',r.creditos);if(p.lema)add('lema',cfg?.lema);return {partes:{...p,pausas:ps},pausas:ps,blocos,texto:blocos.map(b=>b.texto).join('\n'+AUDIRE_TTS_MARCADOR_CAMPO+'\n')};}
function audireTtsPrompt(texto,instrucoes){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes;return `Gere áudio de fala para uma audiodescrição em português brasileiro. Sintetize uma única vez, integralmente e exclusivamente a TRANSCRIÇÃO A FALAR abaixo. Não repita frases, não antecipe nem retome trechos, não acrescente palavras e não leia estas instruções. Mantenha rigorosamente a voz selecionada, o mesmo timbre, a mesma faixa vocal e a mesma impressão de idade. Varie apenas a prosódia necessária ao texto; não troque de locutor.\n\nEstilo de leitura: ${instr}\n\nTRANSCRIÇÃO A FALAR:\n${texto}`;}
function audireGoogleTtsExtrairAudio(payload){
  const raizes=[payload,payload?.interaction].filter(Boolean);
  for(const r of raizes){
    const direto=r?.output_audio?.data||r?.outputAudio?.data;
    if(direto)return direto;
    for(const colecao of [r?.steps,r?.outputs]){
      if(!Array.isArray(colecao))continue;
      for(const passo of colecao){
        const conteudos=Array.isArray(passo?.content)?passo.content:(passo?.content?[passo.content]:[]);
        for(const item of conteudos){
          if(item?.type==='audio'&&item?.data)return item.data;
          if(item?.audio?.data)return item.audio.data;
        }
        if(passo?.type==='audio'&&passo?.data)return passo.data;
      }
    }
    for(const candidato of (Array.isArray(r?.candidates)?r.candidates:[])){
      for(const parte of (Array.isArray(candidato?.content?.parts)?candidato.content.parts:[])){
        const legado=parte?.inlineData?.data||parte?.inline_data?.data;
...
function audireTtsExtrairFrases(trecho){const t=String(trecho||'').trim();if(!t)return[];const out=[];let inicio=0;const abreviacoes=new Set(['sr.','sra.','srta.','dr.','dra.','prof.','profa.','art.','arts.','inc.','incs.','n.','no.','nº.','p.','pp.','ex.','fig.','av.','r.','rod.','etc.','aprox.']);for(let i=0;i<t.length;i++){if(!'.!?…'.includes(t[i]))continue;let fim=i+1;while(fim<t.length&&/[.!?…\"”’')\]]/.test(t[fim]))fim++;const resto=t.slice(fim);if(!(/^\s+/.test(resto)||fim>=t.length))continue;if(t[i]==='.'){const antes=t.slice(inicio,i+1).trim(),ultima=(antes.match(/(?:^|\s)([^\s]+)$/)||[])[1]?.toLowerCase()||'';if(abreviacoes.has(ultima)||/^[a-záàâãéêíóôõúüç]\.$/i.test(ultima))continue;}const frase=t.slice(inicio,fim).trim();if(frase)out.push(frase);inicio=fim;while(inicio<t.length&&/\s/.test(t[inicio]))inicio++;i=inicio-1;}const sobra=t.slice(inicio).trim();if(sobra)out.push(sobra);return out;}
function audireTtsSegmentarTexto(texto,maxChars=650){const fonte=String(texto||'').replace(/\r\n?/g,'\n').trim();if(!fonte)return[];const blocos=fonte.split(AUDIRE_TTS_MARCADOR_CAMPO),unidades=[];for(let bi=0;bi<blocos.length;bi++){const bloco=String(blocos[bi]||'').trim(),partes=bloco.split(/(\n+)/);for(let i=0;i<partes.length;i+=2){const trecho=String(partes[i]||'').trim(),sep=String(partes[i+1]||'');if(!trecho)continue;const frases=audireTtsExtrairFrases(trecho);for(let j=0;j<frases.length;j++){const fimTrecho=j===frases.length-1;let quebra='frase';if(fimTrecho)quebra=sep.length>=2?'linha_branca':'frase';unidades.push({texto:frases[j],quebraApos:quebra,campoIndice:bi});}}if(bi<blocos.length-1&&unidades.length)unidades[unidades.length-1].quebraApos='campo';}
 const segmentos=[];let atual='',quebra='frase';const limite=Math.max(220,Number(maxChars)||650),tolerancia=Math.round(limite*1.35);const fechar=(tipo)=>{if(atual.trim())segmentos.push({texto:atual.trim(),quebraApos:tipo||quebra});atual='';quebra='frase'};for(const u of unidades){if(!atual){atual=u.texto;quebra=u.quebraApos;}else{const teste=atual+' '+u.texto;if(teste.length<=limite){atual=teste;quebra=u.quebraApos;}else{fechar(quebra);atual=u.texto;quebra=u.quebraApos;}}
  if(u.quebraApos==='campo'||u.quebraApos==='linha_branca')fechar(u.quebraApos);
 }
 fechar(quebra);
 // Une apenas um bloco final muito curto, sem jamais quebrar ou duplicar frases e sem atravessar campos/linhas em branco.
 for(let i=segmentos.length-1;i>0;i--){if(['campo','linha_branca'].includes(segmentos[i-1].quebraApos))continue;if(segmentos[i].texto.length<110&&(segmentos[i-1].texto.length+1+segmentos[i].texto.length)<=tolerancia){segmentos[i-1].texto+=' '+segmentos[i].texto;segmentos[i-1].quebraApos=segmentos[i].quebraApos;segmentos.splice(i,1)}}return segmentos;}
function audireTtsPausaEntreSegmentos(tipo,pausas){const p=audireTtsPausasLimpa(pausas);if(tipo==='campo')return p.troca_campo_segundos;if(tipo==='linha_branca')return p.linha_em_branco_segundos;return p.apos_frase_segundos;}
function audirePcm16Medir(pcm){const b=Buffer.from(pcm);let soma=0,n=0,pico=1;for(let i=0;i+1<b.length;i+=2){const v=b.readInt16LE(i),a=Math.abs(v);if(a>220){soma+=v*v;n++}if(a>pico)pico=a}return {rms:n?Math.sqrt(soma/n)/32768:0,pico:pico/32768};}
function audirePcm16AplicarGanho(pcm,ganho){const b=Buffer.from(pcm);if(!Number.isFinite(ganho)||ganho<=1.015)return b;const out=Buffer.allocUnsafe(b.length);for(let i=0;i+1<b.length;i+=2){const v=Math.max(-32768,Math.min(32767,Math.round(b.readInt16LE(i)*ganho)));out.writeInt16LE(v,i)}return out;}
function audireNormalizarFragmentosPcm16(fragmentos,modo='moderada'){const m=audireTtsNormalizacaoLimpa(modo),lista=fragmentos.map(x=>Buffer.from(x));if(m==='desativada'||lista.length<1)return {fragmentos:lista,modo:m,referencia_rms:null};const medidas=lista.map(audirePcm16Medir),maior=Math.max(0,...medidas.map(x=>x.rms));if(maior<=0)return {fragmentos:lista,modo:m,referencia_rms:null};const alvo=m==='forte'?Math.max(maior,.11):maior,maxGanho=m==='forte'?1.60:1.25;const ajustados=lista.map((pcm,i)=>{const x=medidas[i];if(!x.rms||x.rms>=alvo*.985)return pcm;let ganho=Math.min(maxGanho,alvo/x.rms);if(x.pico>0)ganho=Math.min(ganho,.96/x.pico);return audirePcm16AplicarGanho(pcm,ganho)});return {fragmentos:ajustados,modo:m,referencia_rms:Number(alvo.toFixed(5))};}
function audireGoogleTtsInteractionId(payload){return String(payload?.id||payload?.interaction?.id||'').trim();}
async function audireGoogleTtsApagarInteracoes(ids,key,url){for(const id of [...new Set(ids.filter(Boolean))].reverse()){try{await fetch(url+'/'+encodeURIComponent(id),{method:'DELETE',headers:{'x-goog-api-key':key}})}catch{}}}
async function audireGoogleTtsGerarBloco({texto,voice,model,key,url,instrucoes}){const body={model,input:audireTtsPrompt(texto,instrucoes),response_format:{type:'audio'},generation_config:{speech_config:[{voice}]},store:false};let ultimoErro=null;for(let tentativa=1;tentativa<=3;tentativa++){try{const resp=await fetch(url,{method:'POST',headers:{'x-goog-api-key':key,'Content-Type':'application/json'},body:JSON.stringify(body)});const j=await resp.json().catch(()=>({}));if(!resp.ok){let msg=j?.error?.message||j?.message||`Google TTS respondeu HTTP ${resp.status}.`;if(/Input blocked|PROHIBITED_CONTENT|sensitive words/i.test(msg)){if(tentativa<2){ultimoErro=Object.assign(new Error(msg),{status:resp.status});await new Promise(r=>setTimeout(r,450));continue}msg='O classificador do Google bloqueou esta solicitação de síntese. Tente novamente mais tarde ou revise o conteúdo deste fragmento.';}else if(resp.status===400&&/invalid.?argument/i.test(msg))msg='O Google TTS recusou este fragmento como argumento inválido. Tente gerar novamente; se persistir, altere o tamanho máximo do fragmento.';const err=Object.assign(new Error(msg),{status:resp.status});if((resp.status===429||resp.status>=500)&&tentativa<3){ultimoErro=err;await new Promise(r=>setTimeout(r,500*tentativa));continue}throw err}const b64=audireGoogleTtsExtrairAudio(j);if(!b64){const tipos=(j?.steps||j?.interaction?.steps||[]).flatMap(s=>Array.isArray(s?.content)?s.content.map(c=>c?.type).filter(Boolean):[]).join(', ')||'nenhum bloco de áudio identificado';console.error('[Audire TTS] Resposta sem áudio reconhecível.',{model,status:j?.status||j?.interaction?.status||'',tipos});throw Object.assign(new Error('O Google TTS respondeu, mas o Audire não encontrou o bloco de áudio na resposta.'),{status:502,noRetry:true});}const pcm=Buffer.from(b64,'base64');if(!pcm.length)throw Object.assign(new Error('O Google TTS retornou áudio vazio.'),{status:502});const normal=pcm,palavras=String(texto||'').trim().split(/\s+/).filter(Boolean).length,duracao=normal.length/(24000*2),esperada=Math.max(1.5,palavras/2.1);if(palavras>=8&&duracao>Math.max(esperada*2.15,esperada+8)){const err=Object.assign(new Error('O Google TTS devolveu um fragmento com duração anormalmente longa, possível repetição de conteúdo.'),{status:502});if(tentativa<3){ultimoErro=err;await new Promise(r=>setTimeout(r,500*tentativa));continue}throw err}return {pcm:normal,interaction_id:audireGoogleTtsInteractionId(j)}}catch(e){ultimoErro=e;if(e?.noRetry)throw e;if(tentativa<3&&!e.status){await new Promise(r=>setTimeout(r,500*tentativa));continue}if(tentativa>=3||![429,500,502,503,504].includes(Number(e.status)))throw e}}throw ultimoErro||new Error('Falha ao gerar um bloco da síntese de voz.');}
function audireTtsDuracaoSegmentoLimpa(v){const n=Number(v);if(n===0)return 0;return Number.isFinite(n)?Math.max(20,Math.min(50,Math.round(n/5)*5)):30;}
function audireTtsMaxCharsPorSegundos(segundos){const d=audireTtsDuracaoSegmentoLimpa(segundos);return d===0?0:Math.max(220,Math.min(650,Math.round(d*12.5)));}

const AUDIRE_TTS_REVISOES=new Map(),AUDIRE_TTS_REVISAO_TTL_MS=2*60*60*1000,AUDIRE_TTS_REVISAO_BUCKET='audire-tts-revisoes';
function audireTtsRevisaoId(){return require('crypto').randomBytes(18).toString('hex')}
function audireTtsRevisaoLimpar(){const agora=Date.now();for(const [id,x] of AUDIRE_TTS_REVISOES)if(!x||agora-x.atualizado>AUDIRE_TTS_REVISAO_TTL_MS)AUDIRE_TTS_REVISOES.delete(id)}
async function audireTtsGarantirBucketRevisao(){const sb=getSupabase(),{data,error}=await sb.storage.getBucket(AUDIRE_TTS_REVISAO_BUCKET);if(!error&&data)return;const c=await sb.storage.createBucket(AUDIRE_TTS_REVISAO_BUCKET,{public:false,fileSizeLimit:AUDIRE_AUDIO_MAX_BYTES,allowedMimeTypes:['application/json','application/octet-stream']});if(c.error&&!String(c.error.message||'').toLowerCase().includes('already'))throw c.error}
async function audireTtsRevisaoApagarPersistida(id){try{const sb=getSupabase(),{data}=await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).list(String(id),{limit:1000});const paths=(data||[]).map(x=>String(id)+'/'+x.name);if(paths.length)await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).remove(paths)}catch(e){console.warn('[Audire TTS] Não foi possível limpar revisão temporária:',e.message||e)}}
async function audireTtsRevisaoSalvar(s){await audireTtsGarantirBucketRevisao();const sb=getSupabase(),meta={...s,fragmentos:[]};for(let i=0;i<s.fragmentos.length;i++){const f=s.fragmentos[i],fm={...f,versoes:[]};for(let v=0;v<f.versoes.length;v++){const ver=f.versoes[v];if(!ver.storage_path){ver.storage_path=`${s.id}/f${i+1}-v${v+1}-${ver.criada||Date.now()}.pcm`;const up=await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).upload(ver.storage_path,ver.pcm,{contentType:'application/octet-stream',upsert:false});if(up.error)throw up.error}fm.versoes.push({criada:ver.criada,storage_path:ver.storage_path})}meta.fragmentos.push(fm)}meta.expira_em=s.atualizado+AUDIRE_TTS_REVISAO_TTL_MS;const json=Buffer.from(JSON.stringify(meta),'utf8'),path=`${s.id}/sessao.json`,up=await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).upload(path,json,{contentType:'application/json',upsert:true});if(up.error)throw up.error;AUDIRE_TTS_REVISOES.set(s.id,s);return s}
async function audireTtsRevisaoCarregar(id,renovar=true){audireTtsRevisaoLimpar();let s=AUDIRE_TTS_REVISOES.get(String(id));if(!s){try{await audireTtsGarantirBucketRevisao();const sb=getSupabase(),d=await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).download(`${id}/sessao.json`);if(d.error||!d.data)return null;const meta=JSON.parse(Buffer.from(await d.data.arrayBuffer()).toString('utf8'));if(!meta||Date.now()>Number(meta.expira_em||0)){await audireTtsRevisaoApagarPersistida(id);return null}for(const f of meta.fragmentos||[])for(const ver of f.versoes||[]){const a=await sb.storage.from(AUDIRE_TTS_REVISAO_BUCKET).download(ver.storage_path);if(a.error||!a.data)throw a.error||new Error('Fragmento temporário indisponível.');ver.pcm=Buffer.from(await a.data.arrayBuffer())}delete meta.expira_em;s=meta;AUDIRE_TTS_REVISOES.set(String(id),s)}catch(e){console.warn('[Audire TTS] Falha ao restaurar revisão temporária:',e.message||e);return null}}if(Date.now()-Number(s.atualizado||0)>AUDIRE_TTS_REVISAO_TTL_MS){AUDIRE_TTS_REVISOES.delete(String(id));await audireTtsRevisaoApagarPersistida(id);return null}if(renovar){s.atualizado=Date.now();await audireTtsRevisaoSalvar(s)}return s}
async function audireTtsRevisaoAtualizar(s){s.atualizado=Date.now();return audireTtsRevisaoSalvar(s)}
function audireTtsCampoNome(i,nomes){const x=String(nomes?.[i]||('Parte '+(i+1))),m={titulo:'Título',roteiro:'Roteiro',creditos:'Créditos',lema:'Lema'};return m[x]||x}
function audireTtsCampoChave(v){const x=String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();return {titulo:'titulo',roteiro:'roteiro',creditos:'creditos',lema:'lema'}[x]||x}
function audireTtsResumoTexto(t){const x=String(t||'').replace(/\s+/g,' ').trim();return x.length>180?x.slice(0,177)+'…':x}
function audireTtsMontarRevisao(sessao){const raws=sessao.fragmentos.map(f=>f.versoes[f.selecionada].pcm),nivel=audireNormalizarFragmentosPcm16(raws,sessao.normalizacao),partes=[],mapa=[];let segundos=0;for(let i=0;i<sessao.segmentos.length;i++){const seg=sessao.segmentos[i],pcm=nivel.fragmentos[i];partes.push(pcm);segundos+=pcm.length/(24000*2);if(i<sessao.segmentos.length-1){const pausa=audireTtsPausaEntreSegmentos(seg.quebraApos,sessao.pausas);mapa.push({indice:i+1,tipo:seg.quebraApos,posicao_segundos:Number(segundos.toFixed(4)),pausa_segundos:Number(pausa.toFixed(2))});partes.push(audirePcmSilencio(pausa,24000));segundos+=pausa}}return {wav:audireWavDePcm(Buffer.concat(partes),24000,1,16),mapa,normalizacao:nivel.modo}}
function audireTtsRevisaoPublica(sessao){return {id:sessao.id,expira_em:new Date(sessao.atualizado+AUDIRE_TTS_REVISAO_TTL_MS).toISOString(),normalizacao:sessao.normalizacao,campos_presentes:[...new Set(sessao.fragmentos.map(f=>audireTtsCampoChave(f.campo)))],fragmentos:sessao.fragmentos.map((f,i)=>({indice:i+1,campo:f.campo,texto:f.texto,resumo:audireTtsResumoTexto(f.texto),duracao_segundos:Number((f.versoes[f.selecionada].pcm.length/(24000*2)).toFixed(2)),tentativas:f.versoes.length,selecionada:f.selecionada+1}))}}
async function audireTtsGerarBlocoPorProvedor({provedor,texto,voice,model,key,url,instrucoes}){return audireTtsProvedorLimpo(provedor)==='chirp3-hd'?audireChirpTtsGerarBloco({texto,voice}):audireGoogleTtsGerarBloco({texto,voice,model,key,url,instrucoes});}
async function audireChirpTtsGerar({texto,voz,pausas,duracaoSegmentoSegundos,normalizacao,campos}){const voice=audireTtsChirpNomeVoz(voz),model=audireTtsChirpModelo(),duracao_segmento_segundos=audireTtsDuracaoSegmentoLimpa(duracaoSegmentoSegundos),maxChars=audireTtsMaxCharsPorSegundos(duracao_segmento_segundos),segmentos=audireTtsSegmentarTexto(texto,duracao_segmento_segundos===0?4500:Math.min(maxChars,4500));if(!segmentos.length)throw Object.assign(new Error('Não há texto para sintetizar.'),{status:400});const gerados=[];for(const seg of segmentos){const g=await audireChirpTtsGerarBloco({texto:seg.texto,voice});gerados.push(g.pcm)}audireTtsRevisaoLimpar();const id=audireTtsRevisaoId(),sessao={id,atualizado:Date.now(),provedor:'chirp3-hd',model,voice,instrucoes:'',pausas:audireTtsPausasLimpa(pausas),normalizacao:audireTtsNormalizacaoLimpa(normalizacao),segmentos,duracao_segmento_segundos,maxChars:Math.min(maxChars,4500),fragmentos:segmentos.map((seg,i)=>({texto:seg.texto,campo:audireTtsCampoNome(seg.campoIndice,campos),selecionada:0,versoes:[{pcm:gerados[i],criada:Date.now()}]}))};await audireTtsRevisaoSalvar(sessao);const montado=audireTtsMontarRevisao(sessao);if(montado.wav.length>AUDIRE_AUDIO_MAX_BYTES){AUDIRE_TTS_REVISOES.delete(id);await audireTtsRevisaoApagarPersistida(id);throw Object.assign(new Error('A síntese gerada ultrapassa o limite de 20 MB do Audire. Divida o roteiro ou aumente o limite antes de publicar.'),{status:413})}return {wav:montado.wav,provedor:'chirp3-hd',model,voz:voice,instrucoes:'',segmentos:montado.mapa,quantidade_segmentos:segmentos.length,duracao_segmento_segundos,max_chars_segmento:sessao.maxChars,normalizacao:montado.normalizacao,revisao_id:id}}

async function audireGoogleTtsGerar({texto,voz,instrucoes,pausas,duracaoSegmentoSegundos,normalizacao,campos}){const key=audireTtsGoogleApiKey();if(!key)throw Object.assign(new Error('Síntese Google não configurada. Defina GEMINI_API_KEY no Render.'),{status:503});const voice=AUDIRE_TTS_GOOGLE_VOZES.includes(String(voz||''))?String(voz):'Kore',model=audireTtsGoogleModelo(),url='https://generativelanguage.googleapis.com/v1beta/interactions',duracao_segmento_segundos=audireTtsDuracaoSegmentoLimpa(duracaoSegmentoSegundos),maxChars=audireTtsMaxCharsPorSegundos(duracao_segmento_segundos),segmentos=audireTtsSegmentarTexto(texto,duracao_segmento_segundos===0?1000000:maxChars);if(!segmentos.length)throw Object.assign(new Error('Não há texto para sintetizar.'),{status:400});const gerados=[];for(const seg of segmentos){const gerado=await audireGoogleTtsGerarBloco({texto:seg.texto,voice,model,key,url,instrucoes});gerados.push(gerado.pcm)}audireTtsRevisaoLimpar();const id=audireTtsRevisaoId(),sessao={id,atualizado:Date.now(),provedor:'gemini',model,voice,instrucoes:limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,pausas:audireTtsPausasLimpa(pausas),normalizacao:audireTtsNormalizacaoLimpa(normalizacao),segmentos,duracao_segmento_segundos,maxChars,fragmentos:segmentos.map((seg,i)=>({texto:seg.texto,campo:audireTtsCampoNome(seg.campoIndice,campos),selecionada:0,versoes:[{pcm:gerados[i],criada:Date.now()}]}))};await audireTtsRevisaoSalvar(sessao);const montado=audireTtsMontarRevisao(sessao);if(montado.wav.length>AUDIRE_AUDIO_MAX_BYTES){AUDIRE_TTS_REVISOES.delete(id);await audireTtsRevisaoApagarPersistida(id);throw Object.assign(new Error('A síntese gerada ultrapassa o limite de 20 MB do Audire. Divida o roteiro ou aumente o limite antes de publicar.'),{status:413})}return {wav:montado.wav,model,voz:voice,instrucoes:sessao.instrucoes,segmentos:montado.mapa,quantidade_segmentos:segmentos.length,duracao_segmento_segundos,max_chars_segmento:maxChars,normalizacao:montado.normalizacao,revisao_id:id}}


app.get('/audire/admin/tts/revisoes/:sid',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,true);if(!s)return res.status(404).json({error:'A revisão desta síntese expirou ou não está mais disponível. Gere uma nova prévia.'});await audireTtsRevisaoAtualizar(s);res.json({ok:true,revisao:audireTtsRevisaoPublica(s)})});
app.get('/audire/admin/tts/revisoes/:sid/audio',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,true);if(!s)return res.status(404).json({error:'Revisão expirada.'});s.atualizado=Date.now();const m=audireTtsMontarRevisao(s);res.set({'Content-Type':'audio/wav','Cache-Control':'no-store','X-Audire-TTS-Segmentos':encodeURIComponent(JSON.stringify(m.mapa)),'Access-Control-Expose-Headers':'X-Audire-TTS-Segmentos'}).send(m.wav)});
app.get('/audire/admin/tts/revisoes/:sid/fragmentos/:indice/audio',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,true),i=Number(req.params.indice)-1;if(!s||!s.fragmentos[i])return res.status(404).json({error:'Fragmento não encontrado ou revisão expirada.'});const f=s.fragmentos[i],v=Math.max(0,Math.min(f.versoes.length-1,Number(req.query.versao||f.selecionada+1)-1)),pcm=f.versoes[v].pcm;res.set({'Content-Type':'audio/wav','Cache-Control':'no-store'}).send(audireWavDePcm(pcm,24000,1,16))});
app.post('/audire/admin/tts/revisoes/:sid/fragmentos/:indice/regerar',async(req,res)=>{if(!admin(req,res))return;try{const s=await audireTtsRevisaoCarregar(req.params.sid,false),i=Number(req.params.indice)-1;if(!s||!s.fragmentos[i])return res.status(404).json({error:'Fragmento não encontrado ou revisão expirada.'});const key=audireTtsGoogleApiKey(),url='https://generativelanguage.googleapis.com/v1beta/interactions',f=s.fragmentos[i],textoNovo=Object.prototype.hasOwnProperty.call(req.body||{},'texto')?limit(req.body.texto,12000):f.texto;if(!String(textoNovo||'').trim())return res.status(400).json({error:'O texto do fragmento não pode ficar vazio.'});const g=await audireTtsGerarBlocoPorProvedor({provedor:s.provedor||'gemini',texto:textoNovo,voice:s.voice,model:s.model,key,url,instrucoes:s.instrucoes});f.texto=String(textoNovo).trim();s.segmentos[i].texto=f.texto;f.versoes.push({pcm:g.pcm,criada:Date.now()});if(f.versoes.length>2){f.versoes.shift();f.selecionada=Math.max(0,f.selecionada-1)}f.selecionada=f.versoes.length-1;await audireTtsRevisaoAtualizar(s);res.json({ok:true,revisao:audireTtsRevisaoPublica(s)})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao regerar fragmento.'})}});
app.post('/audire/admin/tts/revisoes/:sid/fragmentos/:indice/selecionar',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,false),i=Number(req.params.indice)-1;if(!s||!s.fragmentos[i])return res.status(404).json({error:'Fragmento não encontrado ou revisão expirada.'});const f=s.fragmentos[i],v=Number(req.body?.versao)-1;if(!Number.isInteger(v)||!f.versoes[v])return res.status(400).json({error:'Tentativa inválida.'});f.selecionada=v;await audireTtsRevisaoAtualizar(s);res.json({ok:true,revisao:audireTtsRevisaoPublica(s)})});
app.get('/audire/admin/tts/revisoes/:sid/fragmentos/:indice/transicao',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,true),i=Number(req.params.indice)-1;if(!s||!s.fragmentos[i])return res.status(404).json({error:'Fragmento não encontrado ou revisão expirada.'});const raws=s.fragmentos.map(f=>f.versoes[f.selecionada].pcm),nivel=audireNormalizarFragmentosPcm16(raws,s.normalizacao),partes=[];for(let j=Math.max(0,i-1);j<=Math.min(s.fragmentos.length-1,i+1);j++){let pcm=nivel.fragmentos[j],amostras=Math.round(6*24000)*2;if(j<i)pcm=pcm.subarray(Math.max(0,pcm.length-amostras));else if(j>i)pcm=pcm.subarray(0,Math.min(pcm.length,amostras));partes.push(pcm);if(j<Math.min(s.fragmentos.length-1,i+1))(()=>{const partsPause=audireTtsPausaEntreSegmentos(s.segmentos[j].quebraApos,s.pausas);partes.push(audirePcmSilencio(partsPause,24000))})()}res.set({'Content-Type':'audio/wav','Cache-Control':'no-store'}).send(audireWavDePcm(Buffer.concat(partes),24000,1,16))});

app.post('/audire/admin/tts/revisoes/:sid/normalizacao',async(req,res)=>{if(!admin(req,res))return;const s=await audireTtsRevisaoCarregar(req.params.sid,false);if(!s)return res.status(404).json({error:'Revisão expirada.'});s.normalizacao=audireTtsNormalizacaoLimpa(req.body?.normalizacao);await audireTtsRevisaoAtualizar(s);res.json({ok:true,revisao:audireTtsRevisaoPublica(s)})});
app.post('/audire/admin/tts/revisoes/:sid/partes/adicionar',async(req,res)=>{if(!admin(req,res))return;try{const s=await audireTtsRevisaoCarregar(req.params.sid,false);if(!s)return res.status(404).json({error:'Revisão expirada.'});const cfg=await audireObterConfig(),pedidas=req.body?.partes||{},fontes={titulo:limit(req.body?.titulo,220),roteiro:limit(req.body?.roteiro_texto,50000),creditos:limit(req.body?.creditos,12000),lema:limit(req.body?.lema,300)||cfg.lema},ordem=['titulo','roteiro','creditos','lema'],existentes=new Set(s.fragmentos.map(f=>audireTtsCampoChave(f.campo)));const key=audireTtsGoogleApiKey(),url='https://generativelanguage.googleapis.com/v1beta/interactions';let adicionados=0;for(const tipo of ordem){if(!pedidas[tipo]||existentes.has(tipo)||!String(fontes[tipo]||'').trim())continue;const segs=audireTtsSegmentarTexto(fontes[tipo],s.duracao_segmento_segundos===0?1000000:s.maxChars);const novos=[];for(const seg of segs){const g=await audireTtsGerarBlocoPorProvedor({provedor:s.provedor||'gemini',texto:seg.texto,voice:s.voice,model:s.model,key,url,instrucoes:s.instrucoes});novos.push({seg:{...seg,campoIndice:0},frag:{texto:seg.texto,campo:audireTtsCampoNome(0,[tipo]),selecionada:0,versoes:[{pcm:g.pcm,criada:Date.now()}]}})}const rank=ordem.indexOf(tipo);let pos=s.fragmentos.findIndex(f=>ordem.indexOf(audireTtsCampoChave(f.campo))>rank);if(pos<0)pos=s.fragmentos.length;s.fragmentos.splice(pos,0,...novos.map(x=>x.frag));s.segmentos.splice(pos,0,...novos.map(x=>x.seg));adicionados+=novos.length;existentes.add(tipo)}if(!adicionados)return res.status(400).json({error:'Nenhuma parte nova foi selecionada ou as partes escolhidas já existem nesta revisão.'});for(let i=0;i<s.segmentos.length-1;i++){const a=audireTtsCampoChave(s.fragmentos[i].campo),b=audireTtsCampoChave(s.fragmentos[i+1].campo);if(a!==b)s.segmentos[i].quebraApos='campo'}await audireTtsRevisaoAtualizar(s);res.json({ok:true,revisao:audireTtsRevisaoPublica(s)})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao gerar as partes adicionais.'})}});
function audireTtsHeadersMetadata(req){let instrucoes='';let partes={};try{instrucoes=decodeURIComponent(String(req.headers['x-audire-tts-instrucoes']||''))}catch{}try{partes=JSON.parse(decodeURIComponent(String(req.headers['x-audire-tts-partes']||'{}')))}catch{}const provedor=audireTtsProvedorLimpo(req.headers['x-audire-tts-provedor']);const voz=provedor==='chirp3-hd'?audireTtsChirpNomeVoz(req.headers['x-audire-tts-voz']):(AUDIRE_TTS_GOOGLE_VOZES.includes(String(req.headers['x-audire-tts-voz']||''))?String(req.headers['x-audire-tts-voz']):'Kore');return {provedor:provedor==='chirp3-hd'?'google-cloud-chirp3-hd':'google-gemini-tts',modelo:limit(req.headers['x-audire-tts-modelo'],120)||(provedor==='chirp3-hd'?audireTtsChirpModelo():audireTtsGoogleModelo()),voz,instrucoes:provedor==='chirp3-hd'?'':limit(instrucoes,3000),partes:{...audireTtsPartesLimpa(partes),pausas:audireTtsPausasLimpa(partes?.pausas)}};}
app.get('/audire/public/config',async(req,res)=>{try{res.json({ok:true,config:await audireObterConfig()})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar configuração pública do Audire.'})}});
app.get('/audire/admin/config',async(req,res)=>{if(!admin(req,res))return;try{res.json({ok:true,config:await audireObterConfig()})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar configuração do Audire.'})}});
app.put('/audire/admin/config',async(req,res)=>{if(!admin(req,res))return;try{res.json({ok:true,config:await audireSalvarConfig(req.body||{})})}catch(e){res.status(500).json({error:e.message||'Erro ao salvar configuração do Audire.'})}});

app.get('/audire/public/repositorios',async(req,res)=>{try{const sb=getSupabase(),cfg=await audireObterConfig();const {data,error}=await sb.from('audire_repositorios').select(audirePublicarRepositorioSelect()).eq('status_publicacao','publicado').order('ordem_destaque',{ascending:true,nullsFirst:false}).order('publicado_em',{ascending:false,nullsFirst:false}).order('criado_em',{ascending:false});if(error)throw error;res.json({ok:true,repositorios:(data||[]).filter(r=>audirePaisHabilitado(r,cfg)),config:cfg})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositórios do Audire.'})}});
app.get('/audire/public/repositorios/:id',async(req,res)=>{try{const sb=getSupabase(),cfg=await audireObterConfig();const {data:repo,error}=await sb.from('audire_repositorios').select(audirePublicarRepositorioSelect()).eq('id',req.params.id).eq('status_publicacao','publicado').maybeSingle();if(error)throw error;if(!repo||!audirePaisHabilitado(repo,cfg))return res.status(404).json({error:'Repositório não encontrado.'});const {data:roteiros,error:er}=await sb.from('audire_roteiros').select('id,titulo,ordem,vinculado_em,criado_em').eq('repositorio_id',repo.id).eq('status_publicacao','publicado');if(er)throw er;res.json({ok:true,repositorio:{...repo,roteiros:audireOrdenarRoteirosRepositorio(roteiros)}})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositório do Audire.'})}});
app.post('/audire/public/repositorios/:id/validar-presenca',async(req,res)=>{try{const sb=getSupabase();const {data:repo,error}=await sb.from('audire_repositorios').select('id,status_publicacao,latitude,longitude,exigir_presenca_roteiros,gps_raio_metros,gps_precisao_max_metros').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!repo||repo.status_publicacao!=='publicado')return res.status(404).json({error:'Repositório não encontrado.'});if(!repo.exigir_presenca_roteiros)return res.json({ok:true,token:audireAssinarAcesso(repo.id,'livre'),distancia_metros:0});const lat=Number(req.body?.latitude),lon=Number(req.body?.longitude),accuracy=Math.max(0,Number(req.body?.accuracy)||0);if(!Number.isFinite(lat)||!Number.isFinite(lon))return res.status(400).json({error:'Localização do dispositivo não informada.'});if(!Number.isFinite(Number(repo.latitude))||!Number.isFinite(Number(repo.longitude)))return res.status(409).json({error:'Este repositório exige presença, mas as coordenadas do local não estão configuradas.'});const precisao=Number(repo.gps_precisao_max_metros||500),raio=Number(repo.gps_raio_metros||200);if(accuracy&&accuracy>precisao)return res.status(403).json({error:`A localização está imprecisa demais (${Math.round(accuracy)} m). Tente novamente em local aberto ou mais próximo da exposição.`,codigo:'GPS_IMPRECISO'});const distancia=audireDistanciaMetros(Number(repo.latitude),Number(repo.longitude),lat,lon),ajustada=Math.max(0,distancia-accuracy);if(ajustada>raio)return res.status(403).json({error:`Os roteiros deste repositório estão disponíveis apenas para visitantes presentes no local. Distância aproximada: ${Math.round(distancia)} m.`,codigo:'FORA_DO_RAIO',distancia_metros:Math.round(distancia)});res.json({ok:true,token:audireAssinarAcesso(repo.id,'gps'),distancia_metros:Math.round(distancia),validade_segundos:AUDIRE_PRESENCA_TTL_SEGUNDOS})}catch(e){res.status(500).json({error:e.message||'Erro ao validar presença.'})}});
app.get('/audire/public/roteiros',async(req,res)=>{try{const sb=getSupabase(),cfg=await audireObterConfig();const {data,error}=await sb.from('audire_roteiros').select('id,repositorio_id,titulo,imagem_url,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,criado_em,status_publicacao').eq('status_publicacao','publicado').order('criado_em',{ascending:false});if(error)throw error;const repoIds=[...new Set((data||[]).map(x=>x.repositorio_id).filter(Boolean))];let repos=[];if(repoIds.length){const q=await sb.from('audire_repositorios').select('id,titulo,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,status_publicacao,exigir_presenca_roteiros,ocultar_voltar_roteiro').in('id',repoIds);if(q.error)throw q.error;repos=q.data||[]}const mapa=new Map(repos.map(r=>[r.id,r]));const roteiros=(data||[]).map(r=>{const repo=r.repositorio_id?mapa.get(r.repositorio_id):null;if(r.repositorio_id&&(!repo||repo.status_publicacao!=='publicado'||!audirePaisHabilitado(repo,cfg)))return null;const x=repo?{...r,repositorio_titulo:repo.titulo,categoria:repo.categoria,pais:repo.pais,pais_codigo:repo.pais_codigo,unidade_administrativa:repo.unidade_administrativa,unidade_codigo:repo.unidade_codigo,nome_local:repo.nome_local,endereco:repo.endereco,exigir_presenca_roteiros:!!repo.exigir_presenca_roteiros,ocultar_voltar_roteiro:!!repo.ocultar_voltar_roteiro}:{...r,repositorio_titulo:null,exigir_presenca_roteiros:false,ocultar_voltar_roteiro:false};if(!repo&&Array.isArray(cfg.paises_habilitados)&&x.pais_codigo&&!cfg.paises_habilitados.includes(String(x.pais_codigo).toUpperCase()))return null;return x}).filter(Boolean);res.json({ok:true,roteiros,config:cfg})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar roteiros públicos do Audire.'})}});
app.get('/audire/public/roteiros/:id',async(req,res)=>{try{const sb=getSupabase();const {data:roteiro,error}=await sb.from('audire_roteiros').select('id,repositorio_id,titulo,roteiro_texto,imagem_url,audio_url,creditos,ordem,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,links_adicionais,reproducao_automatica_audio,criado_em,atualizado_em').eq('id',req.params.id).eq('status_publicacao','publicado').maybeSingle();if(error)throw error;if(!roteiro)return res.status(404).json({error:'Roteiro não encontrado.'});if(!roteiro.repositorio_id){const cfg=await audireObterConfig();if(Array.isArray(cfg.paises_habilitados)&&roteiro.pais_codigo&&!cfg.paises_habilitados.includes(String(roteiro.pais_codigo).toUpperCase()))return res.status(404).json({error:'Roteiro não encontrado.'});return res.json({ok:true,roteiro:{...roteiro,repositorio_titulo:null,links_adicionais:roteiro.links_adicionais||[],reproducao_automatica_audio:!!roteiro.reproducao_automatica_audio,ocultar_voltar_roteiro:false}})}const {data:repo,error:er}=await sb.from('audire_repositorios').select('id,titulo,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco,status_publicacao,exigir_presenca_roteiros,links_adicionais,reproducao_automatica_audio,ocultar_voltar_roteiro').eq('id',roteiro.repositorio_id).maybeSingle();if(er)throw er;const cfg=await audireObterConfig();if(!repo||repo.status_publicacao!=='publicado'||!audirePaisHabilitado(repo,cfg))return res.status(404).json({error:'Repositório não encontrado ou não publicado.'});if(repo.exigir_presenca_roteiros){const token=req.headers['x-audire-presenca']||req.query.acesso;if(!audireValidarAcesso(token,repo.id))return res.status(403).json({error:'Este roteiro exige validação de presença no local ou acesso por QR Code.',codigo:'PRESENCA_OBRIGATORIA',repositorio_id:repo.id})}res.json({ok:true,roteiro:{...roteiro,repositorio_titulo:repo.titulo,categoria:repo.categoria,pais:repo.pais,pais_codigo:repo.pais_codigo,unidade_administrativa:repo.unidade_administrativa,unidade_codigo:repo.unidade_codigo,nome_local:repo.nome_local,endereco:repo.endereco,links_adicionais:(Array.isArray(roteiro.links_adicionais)&&roteiro.links_adicionais.length?roteiro.links_adicionais:repo.links_adicionais||[]),reproducao_automatica_audio:!!repo.reproducao_automatica_audio,ocultar_voltar_roteiro:!!repo.ocultar_voltar_roteiro}})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar roteiro do Audire.'})}});

app.get('/audire/q/:codigo',async(req,res)=>{try{const sb=getSupabase();const {data,error}=await sb.from('audire_qr_codes').select('codigo,repositorio_id,roteiro_id,destino_url,ativo').eq('codigo',req.params.codigo).maybeSingle();if(error)throw error;if(!data||data.ativo!==true)return res.status(404).send('QR Code do Audire não encontrado ou inativo.');let destino=String(data.destino_url||'').trim();const base=String(process.env.AUDIRE_PUBLIC_BASE_URL||'').replace(/\/$/,'');if(!destino){if(!base)return res.status(503).send('AUDIRE_PUBLIC_BASE_URL não configurada.');destino=data.roteiro_id?base+'/roteiro.html?id='+encodeURIComponent(data.roteiro_id)+(data.repositorio_id?'&repositorio='+encodeURIComponent(data.repositorio_id):''):base+'/repositorio.html?id='+encodeURIComponent(data.repositorio_id)}if(!/^https?:\/\//i.test(destino))return res.status(400).send('Destino do QR Code inválido.');if(data.repositorio_id&&base&&destino.startsWith(base)){const u=new URL(destino);u.searchParams.set('acesso',audireAssinarAcesso(data.repositorio_id,'qr'));destino=u.toString()}res.redirect(302,destino)}catch(e){res.status(500).send('Erro ao resolver QR Code do Audire.')}});

app.get('/audire/admin/roteiros',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data,error}=await sb.from('audire_roteiros').select('*').order('criado_em',{ascending:false});if(error)throw error;const ids=[...new Set((data||[]).map(x=>x.repositorio_id).filter(Boolean))];let repos=[];if(ids.length){const q=await sb.from('audire_repositorios').select('id,titulo').in('id',ids);if(q.error)throw q.error;repos=q.data||[]}const mapa=new Map(repos.map(r=>[r.id,r.titulo]));res.json({ok:true,roteiros:(data||[]).map(r=>({...r,repositorio_titulo:r.repositorio_id?mapa.get(r.repositorio_id)||null:null}))})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar roteiros.'})}});
app.get('/audire/admin/roteiros/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data,error}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!data)return res.status(404).json({error:'Roteiro não encontrado.'});const {data:qrs,error:eq}=await sb.from('audire_qr_codes').select('*').eq('roteiro_id',data.id).order('criado_em',{ascending:false});if(eq)throw eq;res.json({ok:true,roteiro:{...data,qr_codes:qrs||[]}})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar roteiro.'})}});
app.post('/audire/admin/roteiros',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRoteiro(req.body||{});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro da audiodescrição.'});const repoId=limit(req.body?.repositorio_id,80)||null;let repo=null;if(repoId){const q=await getSupabase().from('audire_repositorios').select('id,pais,pais_codigo,unidade_administrativa,unidade_codigo').eq('id',repoId).maybeSingle();repo=q.data;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});if(!audireRoteiroCompativelRepositorio(b,repo))return res.status(409).json({error:'Este repositório não é compatível com o país e a unidade administrativa do roteiro.'})}const agora=new Date().toISOString();const {data,error}=await getSupabase().from('audire_roteiros').insert({...b,repositorio_id:repoId,vinculado_em:repoId?agora:null,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar roteiro.'})}});
app.get('/audire/admin/repositorios',async(req,res)=>{if(!admin(req,res))return;try{const {data,error}=await getSupabase().from('audire_repositorios').select('*').order('criado_em',{ascending:false});if(error)throw error;res.json({ok:true,repositorios:data||[],config:await audireObterConfig()})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositórios.'})}});
app.post('/audire/admin/repositorios',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRepositorio(req.body||{});if(!b.titulo)return res.status(400).json({error:'Informe o título do repositório.'});if(b.exigir_presenca_roteiros&&(!Number.isFinite(b.latitude)||!Number.isFinite(b.longitude)))return res.status(400).json({error:'Para exigir presença, informe um local com latitude e longitude.'});const agora=new Date().toISOString();if(b.status_publicacao==='publicado')b.publicado_em=agora;const {data,error}=await getSupabase().from('audire_repositorios').insert({...b,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,repositorio:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar repositório.'})}});
app.get('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo,error}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:roteiros,error:er}=await sb.from('audire_roteiros').select('*').eq('repositorio_id',repo.id);if(er)throw er;const {data:qrs,error:eq}=await sb.from('audire_qr_codes').select('*').eq('repositorio_id',repo.id).order('criado_em',{ascending:false});if(eq)throw eq;res.json({ok:true,repositorio:{...repo,roteiros:audireOrdenarRoteirosRepositorio(roteiros),qr_codes:qrs||[]},config:await audireObterConfig()})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositório.'})}});
app.patch('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:antes,error:ea}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(ea)throw ea;if(!antes)return res.status(404).json({error:'Repositório não encontrado.'});const b=audireSanitizarRepositorio({...antes,...(req.body||{})});if(!b.titulo)return res.status(400).json({error:'Informe o título.'});if(b.exigir_presenca_roteiros&&(!Number.isFinite(b.latitude)||!Number.isFinite(b.longitude)))return res.status(400).json({error:'Para exigir presença, informe um local com latitude e longitude.'});const agora=new Date().toISOString(),up={...b,atualizado_em:agora};if(b.status_publicacao==='publicado'&&!antes.publicado_em)up.publicado_em=agora;const {data,error}=await sb.from('audire_repositorios').update(up).eq('id',req.params.id).select().single();if(error)throw error;res.json({ok:true,repositorio:data})}catch(e){res.status(500).json({error:e.message||'Erro ao atualizar repositório.'})}});
app.delete('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),manter=String(req.query.manter_roteiros||'').toLowerCase()==='true';const {data:repo,error:er}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:rots,error:ert}=await sb.from('audire_roteiros').select('*').eq('repositorio_id',req.params.id);if(ert)throw ert;if(manter){for(const r of rots||[]){const upd={repositorio_id:null,vinculado_em:null,atualizado_em:new Date().toISOString()};const q=await sb.from('audire_roteiros').update(upd).eq('id',r.id);if(q.error)throw q.error}await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo.imagem_path);const {error}=await sb.from('audire_repositorios').delete().eq('id',req.params.id);if(error)throw error;return res.json({ok:true,roteiros_preservados:(rots||[]).length})}await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo.imagem_path);for(const r of rots||[]){await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path)}const {error}=await sb.from('audire_repositorios').delete().eq('id',req.params.id);if(error)throw error;res.json({ok:true,roteiros_excluidos:(rots||[]).length})}catch(e){res.status(500).json({error:e.message||'Erro ao excluir repositório.'})}});

app.post('/audire/admin/repositorios/:id/imagem',express.raw({type:'application/octet-stream',limit:'4mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeImagemValido(mime))return res.status(400).json({error:'Use imagem JPEG, PNG ou WebP.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_IMAGE_MAX_BYTES)return res.status(400).json({error:'Imagem inválida ou maior que 3 MB.'});const sb=getSupabase();const {data:repo,error:er}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});await audireGarantirBucket(AUDIRE_REPO_BUCKET,AUDIRE_IMAGE_MAX_BYTES,['image/jpeg','image/png','image/webp']);const path=`${repo.id}/${Date.now()}-${crypto.randomBytes(5).toString('hex')}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_REPO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false,cacheControl:'3600'});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_REPO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_REPO_BUCKET,repo.imagem_path);const alt=limit(decodeURIComponent(String(req.headers['x-image-alt']||'')),500);const {data,error}=await sb.from('audire_repositorios').update({imagem_url:url,imagem_path:path,imagem_alt:alt,atualizado_em:new Date().toISOString()}).eq('id',repo.id).select().single();if(error)throw error;res.json({ok:true,repositorio:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar imagem.'})}});
app.post('/audire/admin/repositorios/:id/vincular-roteiro',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),roteiroId=limit(req.body?.roteiro_id,80);if(!roteiroId)return res.status(400).json({error:'Informe o roteiro avulso.'});const {data:repo}=await sb.from('audire_repositorios').select('id,pais,pais_codigo,unidade_administrativa,unidade_codigo').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:r}=await sb.from('audire_roteiros').select('id,repositorio_id,pais,pais_codigo,unidade_administrativa,unidade_codigo').eq('id',roteiroId).maybeSingle();if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});if(r.repositorio_id)return res.status(409).json({error:'Este roteiro já está vinculado a um repositório.'});if(!audireRoteiroCompativelRepositorio(r,repo))return res.status(409).json({error:'Este roteiro avulso não é compatível com o país e a unidade administrativa deste repositório.'});const agora=new Date().toISOString(),{data,error}=await sb.from('audire_roteiros').update({repositorio_id:repo.id,vinculado_em:agora,atualizado_em:agora}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao vincular roteiro.'})}});
app.post('/audire/admin/repositorios/:id/vincular-roteiros',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),ids=[...new Set((Array.isArray(req.body?.roteiro_ids)?req.body.roteiro_ids:[]).map(x=>limit(x,80)).filter(Boolean))];if(!ids.length)return res.status(400).json({error:'Selecione ao menos um roteiro avulso.'});const {data:repo,error:er}=await sb.from('audire_repositorios').select('id,pais,pais_codigo,unidade_administrativa,unidade_codigo').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:rots,error:ert}=await sb.from('audire_roteiros').select('id,titulo,repositorio_id,pais,pais_codigo,unidade_administrativa,unidade_codigo').in('id',ids);if(ert)throw ert;const mapa=new Map((rots||[]).map(r=>[r.id,r])),invalidos=[];for(const rid of ids){const r=mapa.get(rid);if(!r)invalidos.push('roteiro não encontrado');else if(r.repositorio_id)invalidos.push(r.titulo+' já está vinculado');else if(!audireRoteiroCompativelRepositorio(r,repo))invalidos.push(r.titulo+' não é compatível com a localização')}if(invalidos.length)return res.status(409).json({error:'Não foi possível vincular a seleção: '+invalidos.join('; ')+'.'});const agora=new Date().toISOString(),{data,error}=await sb.from('audire_roteiros').update({repositorio_id:repo.id,vinculado_em:agora,atualizado_em:agora}).in('id',ids).select();if(error)throw error;res.json({ok:true,roteiros:data||[],quantidade:(data||[]).length})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao vincular roteiros.'})}});
app.post('/audire/admin/repositorios/:id/roteiros',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRoteiro(req.body||{});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro da audiodescrição.'});const sb=getSupabase();const {data:repo}=await sb.from('audire_repositorios').select('id,categoria,pais,pais_codigo,unidade_administrativa,unidade_codigo,nome_local,endereco').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});if(!b.pais)b.pais=repo.pais||null;if(!b.pais_codigo)b.pais_codigo=repo.pais_codigo||null;if(!b.unidade_administrativa)b.unidade_administrativa=repo.unidade_administrativa||null;if(!b.unidade_codigo)b.unidade_codigo=repo.unidade_codigo||null;if(!b.nome_local)b.nome_local=repo.nome_local||null;if(!b.endereco)b.endereco=repo.endereco||null;if(!b.categoria||b.categoria==='outros')b.categoria=repo.categoria||'outros';const agora=new Date().toISOString();const {data,error}=await sb.from('audire_roteiros').insert({...b,repositorio_id:repo.id,vinculado_em:agora,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar roteiro.'})}});
app.patch('/audire/admin/roteiros/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:antes,error:ea}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(ea)throw ea;if(!antes)return res.status(404).json({error:'Roteiro não encontrado.'});const b=audireSanitizarRoteiro({...antes,...(req.body||{})});if(!b.titulo||!b.roteiro_texto)return res.status(400).json({error:'Informe título e roteiro.'});let repositorio_id=antes.repositorio_id;if(Object.prototype.hasOwnProperty.call(req.body||{},'repositorio_id'))repositorio_id=limit(req.body?.repositorio_id,80)||null;const mudouRepositorio=repositorio_id!==antes.repositorio_id;if(repositorio_id){const {data:repo,error:er}=await sb.from('audire_repositorios').select('id,pais,pais_codigo,unidade_administrativa,unidade_codigo').eq('id',repositorio_id).maybeSingle();if(er)throw er;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});if(mudouRepositorio&&!audireRoteiroCompativelRepositorio(b,repo))return res.status(409).json({error:'Este repositório não é compatível com o país e a unidade administrativa próprios do roteiro.'});if(!mudouRepositorio&&antes.repositorio_id)b.categoria=antes.categoria||b.categoria}const agora=new Date().toISOString(),vinculado_em=repositorio_id?(mudouRepositorio?agora:(antes.vinculado_em||agora)):null;const {data,error}=await sb.from('audire_roteiros').update({...b,repositorio_id,vinculado_em,atualizado_em:agora}).eq('id',req.params.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao atualizar roteiro.'})}});
app.delete('/audire/admin/roteiros/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:r}=await sb.from('audire_roteiros').select('imagem_path,audio_path').eq('id',req.params.id).maybeSingle();if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const {error}=await sb.from('audire_roteiros').delete().eq('id',req.params.id);if(error)throw error;res.json({ok:true})}catch(e){res.status(500).json({error:e.message||'Erro ao excluir roteiro.'})}});
app.post('/audire/admin/roteiros/:id/imagem',express.raw({type:'application/octet-stream',limit:'4mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeImagemValido(mime))return res.status(400).json({error:'Use imagem JPEG, PNG ou WebP.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_IMAGE_MAX_BYTES)return res.status(400).json({error:'Imagem inválida ou maior que 3 MB.'});const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/imagens/${r.id}-${Date.now()}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.imagem_path);const {data,error}=await sb.from('audire_roteiros').update({imagem_url:url,imagem_path:path,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar imagem do roteiro.'})}});
app.get('/audire/admin/roteiros/:id/audio/fonte',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),{data:r,error}=await sb.from('audire_roteiros').select('audio_path,audio_url,audio_formato').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!r||(!r.audio_path&&!r.audio_url))return res.status(404).json({error:'Este roteiro não possui áudio publicado.'});let buffer=null;if(r.audio_path){const d=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).download(r.audio_path);if(d.error)throw d.error;buffer=Buffer.from(await d.data.arrayBuffer())}else{const fr=await fetch(r.audio_url);if(!fr.ok)throw new Error('Não foi possível obter o áudio publicado.');buffer=Buffer.from(await fr.arrayBuffer())}const f=String(r.audio_formato||'').toLowerCase(),mime=f.includes('ogg')||f.includes('opus')?'audio/ogg':f.includes('mp3')?'audio/mpeg':f.includes('wav')?'audio/wav':f.includes('webm')?'audio/webm':f.includes('m4a')||f.includes('mp4')?'audio/mp4':f.includes('aac')?'audio/aac':'application/octet-stream';res.set({'Content-Type':mime,'Content-Length':String(buffer.length),'Cache-Control':'no-store'}).send(buffer)}catch(e){res.status(500).json({error:e.message||'Erro ao carregar áudio publicado.'})}});
app.post('/audire/admin/roteiros/:id/audio',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const mime=String(req.headers['x-file-mime']||'').toLowerCase();if(!audireMimeAudioValido(mime))return res.status(400).json({error:'Formato de áudio não suportado. Use MP3, WAV, OGG, WebM, M4A ou AAC.'});const buffer=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!buffer.length||buffer.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio inválido ou maior que 20 MB.'});const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/audios/${r.id}-${Date.now()}.${audireExtMime(mime)}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,buffer,{contentType:mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const {data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,audio_origem:'upload',audio_formato:audireExtMime(mime),audio_bitrate_kbps:null,tts_provedor:null,tts_modelo:null,tts_voz:null,tts_instrucoes:null,tts_partes:null,tts_gerado_em:null,atualizado_em:new Date().toISOString()}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data})}catch(e){res.status(500).json({error:e.message||'Erro ao enviar áudio do roteiro.'})}});
app.get('/audire/admin/tts/config',async(req,res)=>{if(!admin(req,res))return;try{const cfg=await audireObterConfig();res.json({ok:true,provedor:'google-gemini-tts',configurado:!!audireTtsGoogleApiKey(),modelo:audireTtsGoogleModelo(),vozes:AUDIRE_TTS_GOOGLE_VOZES,padrao:cfg.tts_padrao||audireTtsPadraoLimpo({}),publicacao:cfg.audio_publicacao||audireAudioPublicacaoLimpa({}),mecanismos:{padrao:'gemini',opcoes:[{id:'gemini',rotulo:'Gemini 3.1',configurado:!!audireTtsGoogleApiKey(),modelo:audireTtsGoogleModelo(),vozes:AUDIRE_TTS_GOOGLE_VOZES,instrucoes:true},{id:'chirp3-hd',rotulo:'Chirp 3 HD',configurado:fs.existsSync(AUDIRE_CLOUD_TTS_CREDENTIALS_FILE),modelo:audireTtsChirpModelo(),vozes:AUDIRE_TTS_CHIRP_VOZES,instrucoes:false}]}})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar configuração de síntese.'})}});
app.post('/audire/admin/roteiros/:id/audio/mix',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Mixagem inválida ou maior que 20 MB.'});const sb=getSupabase(),{data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),preferenciaRoteiro=r.audio_publicacao_formato?audireAudioFormatoLimpo(r.audio_publicacao_formato,r.audio_publicacao_bitrate_kbps):null,formatoHeader=String(req.headers['x-audire-audio-formato']||preferenciaRoteiro?.formato||cfg.audio_publicacao.formato||'mp3').toLowerCase(),formato=formatoHeader==='opus'||formatoHeader==='ogg'?'opus':'mp3',bitrateBase=preferenciaRoteiro?.formato===formato?preferenciaRoteiro.bitrate_kbps:cfg.audio_publicacao.bitrate_kbps,pub=audireAudioFormatoLimpo(formato,req.headers['x-audire-audio-bitrate']||bitrateBase),out=await audireFfmpegConverterAudio(wav,pub.formato,pub.bitrate_kbps);if(out.buffer.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(413).json({error:`O arquivo ${out.ext.toUpperCase()} convertido ultrapassa o limite de 20 MB do Audire.`});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/audios/${r.id}-mix-${Date.now()}-${out.bitrate_kbps}k.${out.ext}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,out.buffer,{contentType:out.mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const agora=new Date().toISOString(),formatoDb=out.formato==='opus'?'ogg-opus':'mp3',{data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,audio_origem:'mixagem',audio_formato:formatoDb,audio_bitrate_kbps:out.bitrate_kbps,atualizado_em:agora}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data,publicacao:{formato:out.formato,extensao:out.ext,mime:out.mime,bitrate_kbps:out.bitrate_kbps,tamanho_bytes:out.buffer.length}})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao publicar mixagem.'})}});
app.post('/audire/admin/roteiros/:id/tts/preview',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),{data:r,error}=await sb.from('audire_roteiros').select('id,titulo,roteiro_texto,creditos').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),fonte={titulo:Object.prototype.hasOwnProperty.call(req.body||{},'titulo')?limit(req.body.titulo,220):r.titulo,roteiro_texto:Object.prototype.hasOwnProperty.call(req.body||{},'roteiro_texto')?limit(req.body.roteiro_texto,50000):r.roteiro_texto,creditos:Object.prototype.hasOwnProperty.call(req.body||{},'creditos')?limit(req.body.creditos,12000):r.creditos},comp=audireTtsComporTexto(fonte,cfg,req.body?.partes||cfg.tts_padrao?.partes,req.body?.pausas||cfg.tts_padrao?.pausas);if(!comp.texto.trim())return res.status(400).json({error:'Selecione ao menos uma parte que contenha texto para gerar a síntese.'});const provedor=audireTtsProvedorLimpo(req.body?.provedor),out=provedor==='chirp3-hd'?await audireChirpTtsGerar({texto:comp.texto,voz:req.body?.voz||'Kore',pausas:comp.pausas,duracaoSegmentoSegundos:req.body?.duracao_segmento_segundos,normalizacao:req.body?.normalizacao??cfg.tts_padrao?.normalizacao,campos:comp.blocos?.map(b=>b.tipo)||[]}):await audireGoogleTtsGerar({texto:comp.texto,voz:req.body?.voz||cfg.tts_padrao?.voz,instrucoes:req.body?.instrucoes||cfg.tts_padrao?.instrucoes,pausas:comp.pausas,duracaoSegmentoSegundos:req.body?.duracao_segmento_segundos,normalizacao:req.body?.normalizacao??cfg.tts_padrao?.normalizacao,campos:comp.blocos?.map(b=>b.tipo)||[]});res.set({'Content-Type':'audio/wav','X-Audire-TTS-Provedor':provedor,'Content-Length':String(out.wav.length),'Cache-Control':'no-store','X-Audire-TTS-Modelo':out.model,'X-Audire-TTS-Voz':out.voz,'X-Audire-TTS-Partes':encodeURIComponent(JSON.stringify(comp.partes)),'X-Audire-TTS-Segmentos':encodeURIComponent(JSON.stringify(out.segmentos||[])),'X-Audire-TTS-Quantidade-Segmentos':String(out.quantidade_segmentos||1),'X-Audire-TTS-Duracao-Segmento':String(out.duracao_segmento_segundos===0?0:(out.duracao_segmento_segundos||30)),'X-Audire-TTS-Normalizacao':out.normalizacao||'moderada','X-Audire-TTS-Revisao':out.revisao_id||'','Access-Control-Expose-Headers':'X-Audire-TTS-Provedor, X-Audire-TTS-Modelo, X-Audire-TTS-Voz, X-Audire-TTS-Partes, X-Audire-TTS-Segmentos, X-Audire-TTS-Quantidade-Segmentos, X-Audire-TTS-Duracao-Segmento, X-Audire-TTS-Normalizacao, X-Audire-TTS-Revisao'}).send(out.wav)}catch(e){const status=Number(e.status)||500;res.status(status).json({error:e.message||'Erro ao gerar prévia de síntese.'})}});
app.post('/audire/admin/audio/convert/:formato',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio WAV inválido ou maior que 20 MB.'});const cfg=await audireObterConfig(),pedido=String(req.params.formato||'').toLowerCase(),formato=pedido==='ogg'||pedido==='opus'?'opus':pedido==='mp3'?'mp3':null;if(!formato)return res.status(400).json({error:'Formato de conversão inválido. Use mp3 ou opus.'});const pub=audireAudioFormatoLimpo(formato,req.headers['x-audire-audio-bitrate']||cfg.audio_publicacao.bitrate_kbps),out=await audireFfmpegConverterAudio(wav,pub.formato,pub.bitrate_kbps);res.set({'Content-Type':out.mime,'Content-Length':String(out.buffer.length),'Cache-Control':'no-store','X-Audire-Audio-Formato':out.formato,'X-Audire-Audio-Bitrate':String(out.bitrate_kbps),'Access-Control-Expose-Headers':'X-Audire-Audio-Formato, X-Audire-Audio-Bitrate'}).send(out.buffer)}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao converter áudio.'})}});
app.post('/audire/admin/audio/convert/mp3',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio WAV inválido ou maior que 20 MB.'});const cfg=await audireObterConfig(),out=await audireFfmpegConverterAudio(wav,'mp3',req.headers['x-audire-audio-bitrate']||cfg.audio_publicacao.bitrate_kbps);res.set({'Content-Type':'audio/mpeg','Content-Length':String(out.buffer.length),'Cache-Control':'no-store','X-Audire-Audio-Bitrate':String(out.bitrate_kbps),'Access-Control-Expose-Headers':'X-Audire-Audio-Bitrate'}).send(out.buffer)}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao converter áudio para MP3.'})}});
app.post('/audire/admin/roteiros/:id/tts/audio',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Prévia de áudio inválida ou maior que 20 MB.'});const sb=getSupabase(),{data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),meta=audireTtsHeadersMetadata(req),preferenciaRoteiro=r.audio_publicacao_formato?audireAudioFormatoLimpo(r.audio_publicacao_formato,r.audio_publicacao_bitrate_kbps):null,formatoHeader=String(req.headers['x-audire-audio-formato']||preferenciaRoteiro?.formato||cfg.audio_publicacao.formato||'mp3').toLowerCase(),formato=formatoHeader==='opus'||formatoHeader==='ogg'?'opus':'mp3',bitrateBase=preferenciaRoteiro?.formato===formato?preferenciaRoteiro.bitrate_kbps:cfg.audio_publicacao.bitrate_kbps,pub=audireAudioFormatoLimpo(formato,req.headers['x-audire-audio-bitrate']||bitrateBase),out=await audireFfmpegConverterAudio(wav,pub.formato,pub.bitrate_kbps);if(out.buffer.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(413).json({error:`O arquivo ${out.ext.toUpperCase()} convertido ultrapassa o limite de 20 MB do Audire.`});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/audios/${r.id}-tts-${Date.now()}-${out.bitrate_kbps}k.${out.ext}`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,out.buffer,{contentType:out.mime,upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const agora=new Date().toISOString(),formatoDb=out.formato==='opus'?'ogg-opus':'mp3',{data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,audio_origem:'sintese',audio_formato:formatoDb,audio_bitrate_kbps:out.bitrate_kbps,tts_provedor:meta.provedor,tts_modelo:meta.modelo,tts_voz:meta.voz,tts_instrucoes:meta.instrucoes,tts_partes:meta.partes,tts_gerado_em:agora,atualizado_em:agora}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data,publicacao:{formato:out.formato,extensao:out.ext,mime:out.mime,bitrate_kbps:out.bitrate_kbps,tamanho_bytes:out.buffer.length}})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao salvar áudio sintetizado.'})}});
function audireCodigoQr(){return crypto.randomBytes(6).toString('base64url').toLowerCase()}
app.post('/audire/admin/repositorios/:id/qr',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo}=await sb.from('audire_repositorios').select('id').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});let codigo=limit(req.body?.codigo,80).toLowerCase().replace(/[^a-z0-9_-]/g,'');if(!codigo)codigo=audireCodigoQr();const payload={codigo,repositorio_id:repo.id,roteiro_id:null,destino_url:limit(req.body?.destino_url,1000)||null,ativo:req.body?.ativo!==false,atualizado_em:new Date().toISOString()};const {data:atual,error:ea}=await sb.from('audire_qr_codes').select('id').eq('repositorio_id',repo.id).is('roteiro_id',null).limit(1).maybeSingle();if(ea)throw ea;let q=atual?await sb.from('audire_qr_codes').update(payload).eq('id',atual.id).select().single():await sb.from('audire_qr_codes').insert({...payload,criado_em:new Date().toISOString()}).select().single();if(q.error)throw q.error;res.json({ok:true,qr:q.data})}catch(e){res.status(500).json({error:e.message||'Erro ao salvar QR Code.'})}});
app.post('/audire/admin/roteiros/:id/qr',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:r,error:er}=await sb.from('audire_roteiros').select('id,repositorio_id').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});let codigo=limit(req.body?.codigo,80).toLowerCase().replace(/[^a-z0-9_-]/g,'');if(!codigo)codigo=audireCodigoQr();const payload={codigo,repositorio_id:r.repositorio_id,roteiro_id:r.id,destino_url:limit(req.body?.destino_url,1000)||null,ativo:req.body?.ativo!==false,atualizado_em:new Date().toISOString()};const {data:atual,error:ea}=await sb.from('audire_qr_codes').select('id').eq('roteiro_id',r.id).limit(1).maybeSingle();if(ea)throw ea;let q=atual?await sb.from('audire_qr_codes').update(payload).eq('id',atual.id).select().single():await sb.from('audire_qr_codes').insert({...payload,criado_em:new Date().toISOString()}).select().single();if(q.error)throw q.error;res.json({ok:true,qr:q.data})}catch(e){res.status(500).json({error:e.message||'Erro ao salvar QR Code do roteiro.'})}});
