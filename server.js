...
function audireAudioPublicacaoLimpa(v){const b=v&&typeof v==='object'?v:{},bitrate=[48,64,96].includes(Number(b.bitrate_kbps))?Number(b.bitrate_kbps):64;return {formato:'mp3',bitrate_kbps:bitrate};}
...
async function audireWavParaMp3(wav,bitrate=64){const info=audireLerWavPcm16(wav),kbps=[48,64,96].includes(Number(bitrate))?Number(bitrate):64;let lame;try{lame=await import('@breezystack/lamejs')}catch(e){const err=new Error('Não foi possível carregar o encoder MP3 no servidor.');err.cause=e;throw err}const Mp3Encoder=lame?.Mp3Encoder||lame?.default?.Mp3Encoder;if(typeof Mp3Encoder!=='function')throw new Error('O módulo MP3 foi carregado, mas não expôs Mp3Encoder pela interface ESM esperada.');const enc=new Mp3Encoder(info.channels,info.sampleRate,kbps),blocos=[],amostras=Math.floor(info.data.length/2/info.channels),left=new Int16Array(amostras),right=info.channels===2?new Int16Array(amostras):null;for(let i=0;i<amostras;i++){left[i]=info.data.readInt16LE((i*info.channels)*2);if(right)right[i]=info.data.readInt16LE((i*info.channels+1)*2)}const passo=1152;for(let i=0;i<amostras;i+=passo){const l=left.subarray(i,Math.min(i+passo,amostras)),r=right?right.subarray(i,Math.min(i+passo,amostras)):null,mp3=r?enc.encodeBuffer(l,r):enc.encodeBuffer(l);if(mp3.length)blocos.push(Buffer.from(mp3))}const fim=enc.flush();if(fim.length)blocos.push(Buffer.from(fim));const out=Buffer.concat(blocos);if(!out.length)throw new Error('Falha ao converter o WAV para MP3.');return {mp3:out,bitrate_kbps:kbps,sample_rate:info.sampleRate,channels:info.channels};}
...
app.post('/audire/admin/audio/convert/mp3',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Áudio WAV inválido ou maior que 20 MB.'});const cfg=await audireObterConfig(),bitrate=[48,64,96].includes(Number(req.headers['x-audire-audio-bitrate']))?Number(req.headers['x-audire-audio-bitrate']):cfg.audio_publicacao.bitrate_kbps,out=await audireWavParaMp3(wav,bitrate);res.set({'Content-Type':'audio/mpeg','Content-Length':String(out.mp3.length),'Cache-Control':'no-store','X-Audire-Audio-Bitrate':String(out.bitrate_kbps),'Access-Control-Expose-Headers':'X-Audire-Audio-Bitrate'}).send(out.mp3)}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao converter áudio para MP3.'})}});
app.post('/audire/admin/roteiros/:id/tts/audio',express.raw({type:'application/octet-stream',limit:'21mb'}),async(req,res)=>{if(!admin(req,res))return;try{const wav=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');if(!wav.length||wav.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(400).json({error:'Prévia de áudio inválida ou maior que 20 MB.'});const sb=getSupabase(),{data:r,error:er}=await sb.from('audire_roteiros').select('*').eq('id',req.params.id).maybeSingle();if(er)throw er;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),meta=audireTtsHeadersMetadata(req),bitrate=[48,64,96].includes(Number(req.headers['x-audire-audio-bitrate']))?Number(req.headers['x-audire-audio-bitrate']):cfg.audio_publicacao.bitrate_kbps,out=await audireWavParaMp3(wav,bitrate);if(out.mp3.length>AUDIRE_AUDIO_MAX_BYTES)return res.status(413).json({error:'O MP3 convertido ultrapassa o limite de 20 MB do Audire.'});await audireGarantirBucket(AUDIRE_ROTEIRO_BUCKET,AUDIRE_AUDIO_MAX_BYTES,['image/jpeg','image/png','image/webp','audio/mpeg','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac','audio/m4a','audio/x-m4a']);const path=`${r.repositorio_id||'avulsos'}/audios/${r.id}-tts-${Date.now()}-${out.bitrate_kbps}k.mp3`;const up=await sb.storage.from(AUDIRE_ROTEIRO_BUCKET).upload(path,out.mp3,{contentType:'audio/mpeg',upsert:false});if(up.error)throw up.error;const url=sb.storage.from(AUDIRE_ROTEIRO_BUCKET).getPublicUrl(path).data.publicUrl;await audireRemoverArquivo(AUDIRE_ROTEIRO_BUCKET,r.audio_path);const agora=new Date().toISOString(),{data,error}=await sb.from('audire_roteiros').update({audio_url:url,audio_path:path,audio_origem:'sintese',audio_formato:'mp3',audio_bitrate_kbps:out.bitrate_kbps,tts_provedor:meta.provedor,tts_modelo:meta.modelo,tts_voz:meta.voz,tts_instrucoes:meta.instrucoes,tts_partes:meta.partes,tts_gerado_em:agora,atualizado_em:agora}).eq('id',r.id).select().single();if(error)throw error;res.json({ok:true,roteiro:data,publicacao:{formato:'mp3',bitrate_kbps:out.bitrate_kbps,tamanho_bytes:out.mp3.length}})}catch(e){res.status(Number(e.status)||500).json({error:e.message||'Erro ao salvar áudio sintetizado.'})}});
