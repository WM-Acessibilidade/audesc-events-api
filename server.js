...
function audireSanitizarRoteiro(b){return {titulo:limit(b.titulo,220),roteiro_texto:limit(b.roteiro_texto,50000),creditos:limit(b.creditos,12000),ordem:Number.isFinite(Number(b.ordem))?Math.trunc(Number(b.ordem)):0,status_publicacao:['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'publicado',categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,links_adicionais:audireSanitizarLinks(b.links_adicionais),reproducao_automatica_audio:b.reproducao_automatica_audio===true||String(b.reproducao_automatica_audio)==='true'};}
