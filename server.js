...
function audireTtsIdadeLimpa(v){const x=String(v||'adulta').trim().toLowerCase();return ['jovem_adulta','adulta','meia_idade','madura'].includes(x)?x:'adulta';}
function audireTtsIdadeDescricao(v){return ({jovem_adulta:'jovem adulta, aproximadamente entre 18 e 29 anos',adulta:'adulta, aproximadamente entre 30 e 45 anos',meia_idade:'de meia-idade, aproximadamente entre 46 e 60 anos',madura:'madura, com mais de 60 anos'})[audireTtsIdadeLimpa(v)];}
function audireTtsPadraoLimpo(v){const b=v&&typeof v==='object'?v:{};return {voz:limit(b.voz,80)||'Kore',idade:audireTtsIdadeLimpa(b.idade),instrucoes:limit(b.instrucoes,3000)||'Leitura clara, natural e acolhedora, em português brasileiro. Ritmo moderado, dicção precisa e tom neutro, próprio para audiodescrição. Evite dramatização excessiva.',partes:audireTtsPartesLimpa(b.partes),pausas:audireTtsPausasLimpa(b.pausas)};}
...
function audireTtsPrompt(texto,instrucoes,idade){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,idadeDesc=audireTtsIdadeDescricao(idade);return `Gere áudio de fala para uma audiodescrição em português brasileiro. Sintetize uma única vez, integralmente e exclusivamente a TRANSCRIÇÃO A FALAR abaixo. Não repita frases, não antecipe nem retome trechos, não acrescente palavras e não leia estas instruções.\n\n# AUDIO PROFILE\nIdentidade: um único audiodescritor virtual, usando rigorosamente a voz selecionada.\nIdade aparente: ${idadeDesc}.\nA identidade vocal é fixa para toda a produção. Em todos os fragmentos, preserve a mesma idade aparente, o mesmo timbre, a mesma altura vocal, a mesma ressonância e o mesmo caráter de voz. Não rejuvenesça nem envelheça o locutor entre fragmentos. Não transforme variações de prosódia em mudança de identidade vocal.\n\n# DIRECTOR'S NOTES\nEstilo de leitura: ${instr}\nConsistência: trate estas características de identidade como invariáveis; varie somente a prosódia necessária ao conteúdo, sem trocar de locutor.\n\n# TRANSCRIÇÃO A FALAR\n${texto}`;}
...
function audireNormalizarPcm16(pcm){const b=Buffer.from(pcm);if(b.length<4)return b;let soma=0,n=0,pico=1;for(let i=0;i+1<b.length;i+=2){const v=b.readInt16LE(i),a=Math.abs(v);if(a>220){soma+=v*v;n++}if(a>pico)pico=a}if(!n)return b;const rms=Math.sqrt(soma/n)/32768,target=.10;let ganho=Math.max(.75,Math.min(1.35,target/Math.max(.0001,rms)));ganho=Math.min(ganho,(32767*.96)/pico);if(Math.abs(ganho-1)<.015)return b;const out=Buffer.allocUnsafe(b.length);for(let i=0;i+1<b.length;i+=2){const v=Math.max(-32768,Math.min(32767,Math.round(b.readInt16LE(i)*ganho)));out.writeInt16LE(v,i)}return out;}
...
app.post('/audire/admin/roteiros/:id/tts/preview',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase(),{data:r,error}=await sb.from('audire_roteiros').select('id,titulo,roteiro_texto,creditos').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!r)return res.status(404).json({error:'Roteiro não encontrado.'});const cfg=await audireObterConfig(),fonte={titulo:Object.prototype.hasOwnProperty.call(req.body||{},'titulo')?limit(req.body.titulo,220):r.titulo,roteiro_texto:Object.prototype.hasOwnProperty.call(req.body||{},'roteiro_texto')?limit(req.body.roteiro_texto,50000):r.roteiro_texto,creditos:Object.prototype.hasOwnProperty.call(req.body||{},'creditos')?limit(req.body.creditos,12000):r.creditos},comp=audireTtsComporTexto(fonte,cfg,req.body?.partes||cfg.tts_padrao?.partes,req.body?.pausas||cfg.tts_padrao?.pausas);if(!comp.texto.trim())return res.status(400).json({error:'Selecione ao menos uma parte que contenha texto para gerar a síntese.'});const out=await audireGoogleTtsGerar({texto:comp.texto,voz:req.body?.voz||cfg.tts_padrao?.voz,idade:req.body?.idade||cfg.tts_padrao?.idade,instrucoes:req.body?.instrucoes||cfg.tts_padrao?.instrucoes,pausas:comp.pausas,duracaoSegmentoSegundos:req.body?.duracao_segmento_segundos});res.set({'Content-Type':'audio/wav','Content-Length':String(out.wav.length),'Cache-Control':'no-store','X-Audire-TTS-Modelo':out.model,'X-Audire-TTS-Voz':out.voz,'X-Audire-TTS-Idade':out.idade,'X-Audire-TTS-Partes':encodeURIComponent(JSON.stringify(comp.partes)),'X-Audire-TTS-Segmentos':encodeURIComponent(JSON.stringify(out.segmentos||[])),'X-Audire-TTS-Quantidade-Segmentos':String(out.quantidade_segmentos||1),'X-Audire-TTS-Duracao-Segmento':String(out.duracao_segmento_segundos===0?0:(out.duracao_segmento_segundos||30)),'Access-Control-Expose-Headers':'X-Audire-TTS-Modelo, X-Audire-TTS-Voz, X-Audire-TTS-Idade, X-Audire-TTS-Partes, X-Audire-TTS-Segmentos, X-Audire-TTS-Quantidade-Segmentos, X-Audire-TTS-Duracao-Segmento'}).send(out.wav)}catch(e){const status=Number(e.status)||500;res.status(status).json({error:e.message||'Erro ao gerar prévia de síntese.'})}});
