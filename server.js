...
function audireTtsPausasLimpa(v){const b=v&&typeof v==='object'?v:{},num=(x,d)=>Number.isFinite(Number(x))?Math.max(0,Math.min(5,Number(x))):d;return {apos_titulo_segundos:num(b.apos_titulo_segundos,1.2),entre_paragrafos_segundos:num(b.entre_paragrafos_segundos,0.8),entre_campos_segundos:num(b.entre_campos_segundos,1.2)};}
...
function audireTtsPrompt(texto,instrucoes,pausas){const instr=limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,ps=audireTtsPausasLimpa(pausas);return `Sua tarefa é sintetizar SOMENTE a transcrição abaixo como fala em português brasileiro. Não vocalize cabeçalhos, nomes de marcadores ou estas instruções. Não acrescente, omita, traduza, resuma nem reescreva o conteúdo. Preserve nomes, números e pontuação.\n\nDireção de voz: ${instr}\n\nDireção de pausas: depois do título, faça uma pausa aproximada de ${ps.apos_titulo_segundos} segundos; entre parágrafos do roteiro, faça uma pausa aproximada de ${ps.entre_paragrafos_segundos} segundos; ao mudar de um campo selecionado para outro, faça uma pausa aproximada de ${ps.entre_campos_segundos} segundos. Quebras de linha em branco indicam pausas naturais; grupos maiores de linhas em branco indicam mudança de campo. Não diga a palavra Audire, texto, transcrição, título, roteiro, créditos ou lema, a menos que uma dessas palavras faça parte literalmente do conteúdo fornecido.\n\nTRANSCRIÇÃO PARA LOCUÇÃO — comece a falar somente depois desta linha:\n${texto}`;}
...
function audireTtsSegmentarTexto(texto,maxChars=650){const fonte=String(texto||'').replace(/\r\n?/g,'\n').trim();if(!fonte)return[];const partes=fonte.split(/(\n{2,})/),unidades=[];for(let i=0;i<partes.length;i+=2){const par=String(partes[i]||'').trim();const sep=String(partes[i+1]||'');if(!par)continue;const frases=par.split(/(?<=[.!?…])\s+(?=[A-ZÁÀÂÃÉÊÍÓÔÕÚÜÇ0-9“"'])/u).map(x=>x.trim()).filter(Boolean);const itens=frases.length?frases:[par];for(let j=0;j<itens.length;j++){for(const pedaco of audireTtsQuebrarUnidadeLonga(itens[j],maxChars)){unidades.push({texto:pedaco,quebraApos:j===itens.length-1?(sep.length>=5?'titulo':sep.length>=4?'campo':sep.length>=2?'paragrafo':'frase'):'frase'});}}}const segmentos=[];let atual='',quebra='frase';const fechar=()=>{if(atual.trim())segmentos.push({texto:atual.trim(),quebraApos:quebra});atual=''};for(const u of unidades){const separador=atual?(u.quebraApos==='paragrafo'||u.quebraApos==='campo'||u.quebraApos==='titulo'?'\n\n':' '):'';const teste=atual+separador+u.texto;if(atual&&teste.length>maxChars){fechar();atual=u.texto}else atual=teste;quebra=u.quebraApos;if(['titulo','campo'].includes(u.quebraApos)&&atual.length>180)fechar()}fechar();return segmentos;}
function audireTtsPausaEntreSegmentos(tipo,pausas){const p=audireTtsPausasLimpa(pausas);if(tipo==='titulo')return p.apos_titulo_segundos;if(tipo==='campo')return p.entre_campos_segundos;if(tipo==='paragrafo')return p.entre_paragrafos_segundos;return .25;}
