...
function audireTtsComporTexto(r,cfg,partes,pausas){const p=audireTtsPartesLimpa(partes),ps=audireTtsPausasLimpa(pausas),blocos=[];const add=(tipo,v)=>{const t=String(v||'').replace(/\r\n?/g,'\n').trim();if(t)blocos.push({tipo,texto:t})};if(p.titulo)add('titulo',r.titulo);if(p.roteiro)add('roteiro',r.roteiro_texto);if(p.creditos)add('creditos',r.creditos);if(p.lema)add('lema',cfg?.lema);return {partes:{...p,pausas:ps},pausas:ps,texto:blocos.map(b=>b.texto).join('\n'+AUDIRE_TTS_MARCADOR_CAMPO+'\n')};}
...
async function audireGoogleTtsGerar({texto,voz,instrucoes,pausas,duracaoSegmentoSegundos,normalizacao}){const key=audireTtsGoogleApiKey();if(!key)throw Object.assign(new Error('Síntese Google não configurada. Defina GEMINI_API_KEY no Render.'),{status:503});const voice=AUDIRE_TTS_GOOGLE_VOZES.includes(String(voz||''))?String(voz):'Kore',model=audireTtsGoogleModelo(),url='https://generativelanguage.googleapis.com/v1beta/interactions',duracao_segmento_segundos=audireTtsDuracaoSegmentoLimpa(duracaoSegmentoSegundos),maxChars=audireTtsMaxCharsPorSegundos(duracao_segmento_segundos),segmentos=audireTtsSegmentarTexto(texto,duracao_segmento_segundos===0?1000000:maxChars);if(!segmentos.length)throw Object.assign(new Error('Não há texto para sintetizar.'),{status:400});const gerados=[];for(const seg of segmentos){const gerado=await audireGoogleTtsGerarBloco({texto:seg.texto,voice,model,key,url,instrucoes});gerados.push(gerado.pcm)}const nivel=audireNormalizarFragmentosPcm16(gerados,normalizacao),partes=[],mapa=[];let segundos=0;for(let i=0;i<segmentos.length;i++){const seg=segmentos[i],pcm=nivel.fragmentos[i];partes.push(pcm);segundos+=pcm.length/(24000*2);if(i<segmentos.length-1){const pausa=audireTtsPausaEntreSegmentos(seg.quebraApos,pausas);mapa.push({indice:i+1,tipo:seg.quebraApos,posicao_segundos:Number(segundos.toFixed(4)),pausa_segundos:Number(pausa.toFixed(2))});const silencio=audirePcmSilencio(pausa,24000);partes.push(silencio);segundos+=pausa}}const wav=audireWavDePcm(Buffer.concat(partes),24000,1,16);if(wav.length>AUDIRE_AUDIO_MAX_BYTES)throw Object.assign(new Error('A síntese gerada ultrapassa o limite de 20 MB do Audire. Divida o roteiro ou aumente o limite antes de publicar.'),{status:413});return {wav,model,voz:voice,instrucoes:limit(instrucoes,3000)||audireTtsPadraoLimpo({}).instrucoes,segmentos:mapa,quantidade_segmentos:segmentos.length,duracao_segmento_segundos,max_chars_segmento:maxChars,normalizacao:nivel.modo}}
