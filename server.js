...
// -----------------------------------------------------------------------------
// Audire Fase 1.1 — administração, uploads, roteiros e QR dinâmico.
// Rotas isoladas em /audire/admin/* e buckets exclusivos audire-*.
// -----------------------------------------------------------------------------
...
async function audireGarantirBucket(nome,limite,mimes){
  const sb=getSupabase();
  const {data,error}=await sb.storage.getBucket(nome);
  if(!error&&data)return;
  const criado=await sb.storage.createBucket(nome,{public:true,fileSizeLimit:limite,allowedMimeTypes:mimes});
  if(criado.error&&!String(criado.error.message||'').toLowerCase().includes('already'))throw criado.error;
}
...
function audireSanitizarRepositorio(b){
  const status=['rascunho','publicado','oculto'].includes(String(b.status_publicacao||''))?String(b.status_publicacao):'rascunho';
  return {titulo:limit(b.titulo,220),descricao:limit(b.descricao,12000),categoria:limit(b.categoria,100)||'outros',pais:limit(b.pais,120)||null,pais_codigo:limit(b.pais_codigo,12)||null,unidade_administrativa:limit(b.unidade_administrativa,160)||null,unidade_codigo:limit(b.unidade_codigo,30)||null,nome_local:limit(b.nome_local,220)||null,endereco:limit(b.endereco,500)||null,latitude:Number.isFinite(Number(b.latitude))?Number(b.latitude):null,longitude:Number.isFinite(Number(b.longitude))?Number(b.longitude):null,google_place_id:limit(b.google_place_id,220)||null,imagem_alt:limit(b.imagem_alt,500)||null,status_publicacao:status};
}
...
app.get('/audire/admin/repositorios',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data,error}=await sb.from('audire_repositorios').select('*').order('criado_em',{ascending:false});if(error)throw error;res.json({ok:true,repositorios:data||[]})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositórios.'})}});
app.post('/audire/admin/repositorios',async(req,res)=>{if(!admin(req,res))return;try{const b=audireSanitizarRepositorio(req.body||{});if(!b.titulo)return res.status(400).json({error:'Informe o título do repositório.'});const agora=new Date().toISOString();if(b.status_publicacao==='publicado')b.publicado_em=agora;const {data,error}=await getSupabase().from('audire_repositorios').insert({...b,criado_em:agora,atualizado_em:agora}).select().single();if(error)throw error;res.json({ok:true,repositorio:data})}catch(e){res.status(500).json({error:e.message||'Erro ao criar repositório.'})}});
app.get('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo,error}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(error)throw error;if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});const {data:roteiros,error:er}=await sb.from('audire_roteiros').select('*').eq('repositorio_id',repo.id).order('ordem',{ascending:true}).order('criado_em',{ascending:true});if(er)throw er;const {data:qrs,error:eq}=await sb.from('audire_qr_codes').select('*').eq('repositorio_id',repo.id).order('criado_em',{ascending:false});if(eq)throw eq;res.json({ok:true,repositorio:{...repo,roteiros:roteiros||[],qr_codes:qrs||[]}})}catch(e){res.status(500).json({error:e.message||'Erro ao carregar repositório.'})}});
app.patch('/audire/admin/repositorios/:id',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:antes,error:ea}=await sb.from('audire_repositorios').select('*').eq('id',req.params.id).maybeSingle();if(ea)throw ea;if(!antes)return res.status(404).json({error:'Repositório não encontrado.'});const b=audireSanitizarRepositorio({...antes,...(req.body||{})});if(!b.titulo)return res.status(400).json({error:'Informe o título.'});const agora=new Date().toISOString();const up={...b,atualizado_em:agora};if(b.status_publicacao==='publicado'&&!antes.publicado_em)up.publicado_em=agora;const {data,error}=await sb.from('audire_repositorios').update(up).eq('id',req.params.id).select().single();if(error)throw error;res.json({ok:true,repositorio:data})}catch(e){res.status(500).json({error:e.message||'Erro ao atualizar repositório.'})}});
...
app.post('/audire/admin/repositorios/:id/qr',async(req,res)=>{if(!admin(req,res))return;try{const sb=getSupabase();const {data:repo}=await sb.from('audire_repositorios').select('id').eq('id',req.params.id).maybeSingle();if(!repo)return res.status(404).json({error:'Repositório não encontrado.'});let codigo=limit(req.body?.codigo,80).toLowerCase().replace(/[^a-z0-9_-]/g,'');if(!codigo)codigo=audireCodigoQr();const payload={codigo,repositorio_id:repo.id,destino_url:limit(req.body?.destino_url,1000)||null,ativo:req.body?.ativo!==false,atualizado_em:new Date().toISOString()};const {data:atual,error:ea}=await sb.from('audire_qr_codes').select('id').eq('repositorio_id',repo.id).order('criado_em',{ascending:true}).limit(1).maybeSingle();if(ea)throw ea;let q;if(atual){q=await sb.from('audire_qr_codes').update(payload).eq('id',atual.id).select().single()}else{q=await sb.from('audire_qr_codes').insert({...payload,criado_em:new Date().toISOString()}).select().single()}if(q.error)throw q.error;res.json({ok:true,qr:q.data})}catch(e){res.status(500).json({error:e.message||'Erro ao salvar QR Code.'})}});
...
app.get('/audire/public/qr/:codigo.svg',async(req,res)=>{try{const sb=getSupabase();const {data,error}=await sb.from('audire_qr_codes').select('codigo,ativo').eq('codigo',req.params.codigo).maybeSingle();if(error)throw error;if(!data||data.ativo!==true)return res.status(404).send('QR Code não encontrado.');const QRCode=require('qrcode');const base=String(process.env.AUDIRE_API_PUBLIC_URL||process.env.RENDER_EXTERNAL_URL||('https://audesc-events-api.onrender.com')).replace(/\/$/,'');const url=base+'/audire/q/'+encodeURIComponent(req.params.codigo);const svg=await QRCode.toString(url,{type:'svg',margin:2,width:360,errorCorrectionLevel:'M'});res.type('image/svg+xml').set('Cache-Control','public, max-age=3600').send(svg)}catch(e){res.status(500).send('Erro ao gerar QR Code.')}});
